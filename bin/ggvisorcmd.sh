...
CP="${GRIDGAIN_LIBS}${SEP}${GRIDGAIN_HOME}/bin/include/gridgain-visor-console/*"
...
case $osname in
    Darwin*)
        "$JAVA" ${JVM_OPTS} ${QUIET} "${DOCK_OPTS}" -DVISOR -DVISOR_REPL -DGRIDGAIN_SCRIPT \
    ...
    ;;
    *)
        "$JAVA" ${JVM_OPTS} ${QUIET} -DVISOR -DVISOR_REPL -DGRIDGAIN_SCRIPT -DGRIDGAIN_UPDATE_NOTIFIER=false \
