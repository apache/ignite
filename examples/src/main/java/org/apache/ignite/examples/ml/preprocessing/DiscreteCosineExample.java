...
import org.apache.ignite.ml.preprocessing.dct.DiscreteCosineTrainer;

import java.io.Serializable;

/**
 * Example that shows how to use Discrete Cosine preprocessor to transform each vector in the given data.
 * <p>
 * Code in this example launches Ignite grid and fills the cache with simple test data.</p>
 * <p>
 * After that it defines preprocessors that extract features from an upstream data and applies Discrete Cosine Transformation to their values.</p>
 * <p>
 ...
 */
public class DiscreteCosineExample {
    /** Run example. */
    public static void main(String[] args) throws Exception {
        try (Ignite ignite = Ignition.start("examples/config/example-ignite.xml")) {
            System.out.println(">>> Discrete cosine example started.");
            ...
            try {
                ...
                System.out.println(">>> Discrete cosine example completed.");
            } finally {
                data.destroy();
            }
        }
    ...
    private static IgniteCache<Integer, Vector> createCache(Ignite ignite) {
        ...
        persons.put(2, new DenseVector(new Serializable[]{"John", 20, 10, 500}));
        persons.put(3, new DenseVector(new Serializable[]{"George", 30, 0, 10}));
