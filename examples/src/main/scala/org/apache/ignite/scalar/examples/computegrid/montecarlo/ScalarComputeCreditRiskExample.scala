...
object ScalarComputeCreditRiskExample {
    def main(args: Array[String]) {
        scalar("examples/config/example-ignite.xml") {
            ...
            val crdRisk = ignite$ @< (closures(cluster$.nodes().size(), portfolio, horizon, it, percentile),
     ...
     * @param it Number of Monte-Carlo iterations.
     * @param percentile Percentile.
     * @return Collection of closures.
     */
    private def closures(clusterSize: Int, portfolio: Seq[Credit], horizon: Int, it: Int,
        percentile: Double): Seq[() => Double] = {
        val itPerNode = math.round(it / clusterSize.asInstanceOf[Float])
        val lastNodeIt = it - (clusterSize - 1) * itPerNode

        var cls = Seq.empty[() => Double]

        (0 until clusterSize).foreach(i => {
            val nodeIt = if (i == clusterSize - 1) lastNodeIt else itPerNode

            cls +:= (() => new CreditRiskManager().calculateCreditRiskMonteCarlo(
                portfolio, horizon, nodeIt, percentile))
        })
 ...
 */
private case class Credit(remAmnt: Double, remTerm: Int, apr: Double, edf: Double) {
    /**
...
private class CreditRiskManager {
     ...
     */
    def calculateCreditRiskMonteCarlo(portfolio: Seq[Credit], horizon: Int, num: Int, percentile: Double): Double = {
