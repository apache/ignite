...
object ScalarStarSchemaExample {
    ...
    def main(args: Array[String]) {
        scalar(CONFIG) {
            ...
            val dimCache = createCache$[JavaInt, AnyRef](REPLICATED_CACHE_NAME, CacheMode.REPLICATED,
                Seq(classOf[JavaInt], classOf[DimStore], classOf[JavaInt], classOf[DimProduct]))

            try {
                val factCache = createCache$[JavaInt, FactPurchase](PARTITIONED_CACHE_NAME,
                    indexedTypes = Seq(classOf[JavaInt], classOf[FactPurchase]))
    ...
    def populateFacts(factCache: IgniteCache[JavaInt, FactPurchase]) {
        for (i <- 1 to 100) {
            val store = rand(dataStore.values)
            val prod = rand(dataProduct.values)
            val purchase = new FactPurchase(idGen.next(), prod.id, store.id, i + 1)
    ...
    private def queryProductPurchases() {
        ...
        // =================================================
        val p1 = rand(dataProduct.values)
        val p2 = rand(dataProduct.values)
        val p3 = rand(dataProduct.values)
     ...
     *
     * @param c Input collection (no `null` and not empty).
