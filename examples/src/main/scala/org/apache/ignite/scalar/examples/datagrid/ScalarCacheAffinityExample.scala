...
object ScalarCacheAffinityExample extends App {
    ...
    /** Name of cache. */
    private val CACHE_NAME = ScalarCacheAffinityExample.getClass.getSimpleName
     ...
     */
    scalar(CONFIG) {
        val cache = createCache$[Int, String](CACHE_NAME)
    ...
    private def visitUsingAffinityRun(c: IgniteCache[Int, String]) {
        (0 until KEY_CNT).foreach (i =>
            ignite$.compute ().affinityRun (CACHE_NAME, i,
    ...
    private def visitUsingMapKeysToNodes(c: IgniteCache[Int, String]) {
        val keys = 0 until KEY_CNT

        val cluster = cluster$

        // Map all keys to nodes.
        val mappings = ignite$.affinity(CACHE_NAME).mapKeysToNodes(keys)
