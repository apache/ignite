...
public class EventsApiExample {
    ...
    private static void localListen() throws Exception {
        ...
        GridPredicate<GridTaskEvent> lsnr = (GridPredicate<GridTaskEvent>) evt -> {
        ...
        g.compute().withName("example-event-task").run(
            (GridRunnable)() -> System.out.println("Executing sample job.")
        ).get();
    ...
    private static void remoteListen() throws GridException {
        ...
        GridBiPredicate<UUID, GridTaskEvent> locLsnr = (GridBiPredicate<UUID, GridTaskEvent>) (nodeId, evt) -> {
        ...
        GridPredicate<GridTaskEvent> rmtLsnr =
            (GridPredicate<GridTaskEvent>) evt -> evt.taskName().startsWith("good-task");
