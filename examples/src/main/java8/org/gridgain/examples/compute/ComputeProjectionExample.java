...
public class ComputeProjectionExample {
    ...
    public static void main(String[] args) throws Exception {
        try (Grid grid = GridGain.start("examples/config/example-compute.xml")) {
            System.out.println(grid.configuration().getDeploymentMode());

            int prjSize = grid.nodes().size();
            ...
            sayHello(grid.forPredicate((GridPredicate<GridNode>) n -> n.metrics().getCurrentCpuLoad() < 0.5));
        }
    }
    ...
    private static void sayHello(final GridProjection g) throws GridException {
        ...
        g.forRemotes().compute().broadcast(
            (GridRunnable)() -> System.out.println(">>> Hello Node: " + g.grid().localNode().id())
        ).get();
    }
}
