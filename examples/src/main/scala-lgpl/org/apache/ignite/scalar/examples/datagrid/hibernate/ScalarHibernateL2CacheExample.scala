...
object ScalarHibernateL2CacheExample extends App {
    ...
    private val ENTITY_NAMES =
        JavaArrays.asList(classOf[User].getName, classOf[Post].getName, classOf[User].getName + ".posts")
    ...
    /** Caches' names. */
    private val UPDATE_TIMESTAMPS_CACHE_NAME = "org.hibernate.cache.spi.UpdateTimestampsCache"
    private val STANDART_QUERY_CACHE_NAME = "org.hibernate.cache.internal.StandardQueryCache"
    private val USER_CACHE_NAME = "org.apache.ignite.scalar.examples.datagrid.hibernate.User"
    private val USER_POSTS_CACHE_NAME = "org.apache.ignite.scalar.examples.datagrid.hibernate.User.posts"
    private val POST_CACHE_NAME = "org.apache.ignite.scalar.examples.datagrid.hibernate.Post"
    ...
    scalar(CONFIG) {
        ...
        val c1 = createCache(UPDATE_TIMESTAMPS_CACHE_NAME, ATOMIC)
        val c2 = createCache(STANDART_QUERY_CACHE_NAME, ATOMIC)
        val c3 = createCache(USER_CACHE_NAME, TRANSACTIONAL)
        val c4 = createCache(USER_POSTS_CACHE_NAME, TRANSACTIONAL)
        val c5 = createCache(POST_CACHE_NAME, TRANSACTIONAL)

        try {
            val hibernateCfg = ExamplesUtils.url(HIBERNATE_CFG)

            val sesFactory = createHibernateSessionFactory(hibernateCfg)
    ...
    /**
      ...
      */
    private def createCache[K, V](name: String, atomicityMode: CacheAtomicityMode): IgniteCache[K, V] = {
        val ccfg = new CacheConfiguration[K, V](name)
    ...
    /**
      ...
      *
      * @param sesFactory Hibernate { @link SessionFactory}, for which to print statistics.
      */
