...
package org.gridgain.examples.advanced.misc.client.memcache;
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (Grid g = GridGain.start("examples/config/example-cache-client-memcache.xml")) {
            ...
            if (client.add("personKey", 0, new Person("John", "Smith", 1500.0, "Bachelor degree")).get())
                System.out.println(">>> Successfully put person using Memcache client.");

            // Check that Person is actually in cache using traditional
            // GridGain API and Memcache binary protocol.
            System.out.println(">>> Getting value for 'personKey' using GridGain cache API: " + cache.get("personKey"));
            System.out.println(">>> Getting value for 'personKey' using Memcache client: " + client.get("personKey"));

            // Remove Person from cache using Memcache binary protocol.
            if (client.delete("personKey").get())
                System.out.println(">>> Successfully removed person using Memcache client.");
