...
export const colsToGrid = function (cols) {
  ...
  for (const key in cols) {
    rets[key] = cols[key].constructor.name === 'Array'
      ? cols[key].filter((col) => col.toString().substr(0,14) === 'igfs-internal-' && col.toString().substr(-5) === '-meta'
          .filter((value) => cols[key].includes(value)))
        .map((col) => col.toString().slice(14, -5))
        .sort()
      : [];
  }
  return rets;
