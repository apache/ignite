  ...
  _.each(rets, (val, key) => {
    rets[key] = _.map(
      _.filter(rets[key], col =>
		...
		|| col.toString().substr(0,14) === 'igfs-internal-' && col.toString().substr(-5) === '-meta'
      ),
      col => {
		  if(col.toString().substr(0,14) === 'igfs-internal-'){
			return col.toString().slice(14,-5)
		  }
		  else{
			return col.toString().slice(0, -6)
		  }
	  }
    ).sort();
  });
  
  // hidden igfs-internal-bucket-data  

  console.log('colsToGrid:',rets);
  return rets;
};
