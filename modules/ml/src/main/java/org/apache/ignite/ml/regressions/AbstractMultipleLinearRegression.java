...
public abstract class AbstractMultipleLinearRegression implements MultipleLinearRegression {
     ...
     * {@code data} array containing the sequence of values {@code (1, 2, 3, 4, 5, 6, 7, 8, 9)} with
     * {@code nobs = 3} and {@code nvars = 2} creates a regression dataset with two
     ...
     * <ul><li>{@code data} cannot be {@code null}</li>
     ...
     * @param nvars Number of independent variables (columns, not counting {@code y}).
     * @param like Matrix (maybe empty) indicating how data should be stored.
     * @throws NullArgumentException If the data array is null.
     * @throws CardinalityException If the length of the data array is not equal to {@code nobs * (nvars + 1)}.
     * @throws InsufficientDataException If {@code nobs} is less than {@code nvars + 1}.
     */
     ...
     * </p>
     * The input {@code x} array should have one row for each sample
     * observation, with columns corresponding to independent variables.
     * For example, if <pre>
     * {@code x = new double[][] {{1, 2}, {3, 4}, {5, 6}} }</pre>
     * then {@code setXSampleData(x)} results in a model with two independent
     ...
     * </p>
     *
     * @param x The rectangular array representing the {@code x} sample.
     * @throws NullArgumentException If {@code x} is null.
     * @throws NoDataException If {@code x} is empty.
     ...
     * <li>The length (i.e. number of rows) of {@code x} equals the length of {@code y}.</li>
     * <li>{@code x} has at least one more row than it has columns (i.e. there is
     * sufficient data to estimate regression coefficients for each of the
     * columns in x plus an intercept.</li>
     * </ul>
     *
     * @param x The {@code n x k} matrix representing the {@code x} data.
     * @param y The {@code n}-sized vector representing the {@code y} data.
    ...
    /**
     * Validates that the {@code x} data and covariance matrix have the same
