...
import org.apache.ignite.ml.selection.mapper.SHA256UniformMapper;
import org.apache.ignite.ml.selection.mapper.UniformMapper;
...
public class TrainTestDatasetSplitter<K, V> implements Serializable {
    ...
    public TrainTestSplit<K, V> split(double trainSize, double testSize) {
        return new TrainTestSplit<>(
            (k, v) -> {
                double val = mapper.map(k, v);
                return val <= trainSize;
            },
            (k, v) -> {
                double val = mapper.map(k, v);
                return val > trainSize && val <= (trainSize + testSize);
            }
        );
    }
}
