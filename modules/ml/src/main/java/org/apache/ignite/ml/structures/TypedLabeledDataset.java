...
public class TypedLabeledDataset<M extends Matrix, L extends Vector> {
     ...
     * @param lbs Labels.
     */
    public TypedLabeledDataset(M matrix, L lbs) {
        this.matrix = matrix;
        this.lbs = lbs;
    }
     ...
     * @return
     */
    public static TypedLabeledDataset<Matrix, Vector> loadTxt(String pathToFile, String separator) {
        ...
        try (Stream<String> stream = Files.lines(Paths.get(pathToFile))) {
            ...
            if(rowSize > 0){
                ...
                return new TypedLabeledDataset<>(mtx, labels);
