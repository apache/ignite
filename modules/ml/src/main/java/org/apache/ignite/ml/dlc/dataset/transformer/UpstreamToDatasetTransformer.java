...
/**
 * Transforms upstream data into dataset recoverable part of partition using specified feature extractor.
 *
 * @param <K> type of an upstream value key
 * @param <V> type of an upstream value
 */
public class UpstreamToDatasetTransformer<K, V> implements DLCPartitionRecoverableDataTransformer<K, V, Serializable, DLCDatasetPartitionRecoverable> {
    /** */
     ...
     */
    protected UpstreamToDatasetTransformer(
    ...
    /** {@inheritDoc} */
    @Override public DLCDatasetPartitionRecoverable apply(Iterable<Cache.Entry<K, V>> entries, Long aLong, Serializable serializable) {
        double[][] features = new double[Math.toIntExact(aLong)][];
        int ptr = 0;
        for (Cache.Entry<K, V> e : entries)
            features[ptr++] = featureExtractor.apply(e);
        return new DLCDatasetPartitionRecoverable(features);
    }
}
