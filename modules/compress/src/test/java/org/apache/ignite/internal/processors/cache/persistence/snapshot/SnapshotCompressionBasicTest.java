    ...
    @Override protected void cleanPersistenceDir(boolean saveSnap) {
        ...
        try (DirectoryStream<Path> ds = Files.newDirectoryStream(defaultWorkDirectory(),
            ...
            for (Path dir : ds) {
                ...
                log.info("************** START");
                log.info("ВСЕ ДИРЕКТОРИИ - ft.allStorages()");
                try {
                    log.info("----------------------------------------------------------------------------------");
                    for (File f : ft.allStorages().collect(Collectors.toList())) {
                        logDirectoryContent(f, "", 0, null);
                    }
                    log.info("----------------------------------------------------------------------------------");
                }
                catch (Exception e) {
                ...
                log.info("**************");
                log.info("Dir cacheStorage paths: ");
                log.info("**************");
                log.info("Dir temporary cacheStorage paths: ");
                try {
                    for (File f : ft.existingTmpCacheStorages()) {
