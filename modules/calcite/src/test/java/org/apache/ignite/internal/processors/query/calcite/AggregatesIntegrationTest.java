...
public class AggregatesIntegrationTest extends AbstractBasicIntegrationTest {
    /** */
    @Test
    public void useCorrectTypeOfAggregates() throws InterruptedException {
        IgniteCache<Integer, Employer> table = createAndPopulateTable();

        Matcher<String> mapReduceHashAggregates = QueryChecker.containsSubPlan("IgniteReduceHashAggregate");
        Matcher<String> hashAggregates = QueryChecker.containsSubPlan("IgniteHashAggregate");

        // seems could use map-reduce after statistics will be integrated
        assertQuery("select count(*) from person where salary > 0")
            .matches(hashAggregates)
            .check();

        assertQuery("select count(*) from person")
            .matches(mapReduceHashAggregates)
            .check();

        assertQuery("select count(_key) from person")
            .matches(mapReduceHashAggregates)
            .check();
        ...
        assertQuery("select avg(salary), name from person group by name")
            .matches(mapReduceHashAggregates)
            .check();

        // Populate more data
        for (int i = 0; i < 100; i++)
            table.put(Integer.MAX_VALUE - i, new Employer("Roma", 10d));

        // Need to refresh table statistics (as of now refresh statistics depends on number of invocation
        for (int i = 0; i < 10; i++) {
            assertQuery("select count(*) from person where salary > 0").check();
