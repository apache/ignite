...
abstract class AbstractJoinExecutionTest extends AbstractExecutionTest {
    /** @see #doTestJoinBuffer(JoinRelType, JoinCreator, Consumer) */
    protected abstract JoinCreator joinCreator();

    /** @see #doTestJoinBuffer(JoinRelType, JoinCreator, Consumer) */
    protected abstract Consumer<AbstractNode<?>> joinFinalChecker();

    /** Tests join of certain type with input bigger that the buffer size. */
    @Test
    public void testJoinBufer() throws Exception {
        for (JoinRelType joinType : F.asList(LEFT, INNER, RIGHT, FULL, SEMI, ANTI)) {
            if (log.isInfoEnabled())
                log.info("Testing join of type '" + joinType + "'...");

            doTestJoinBuffer(joinType, joinCreator(), joinFinalChecker());
        }
    }

    /**
     * @param jType Join type.
     * @param jCreator Creates certain join node.
     * @param finalChecker Finally check node after successfull run
     */
    private void doTestJoinBuffer(JoinRelType jType, JoinCreator jCreator, Consumer<AbstractNode<?>> finalChecker) throws Exception {
        ...
        AbstractNode<Object[]> join = jCreator.create(ctx, outType, leftType, rightType, jType,
        ...
        finalChecker.accept(join);
    }

    /** */
    @FunctionalInterface
    protected interface JoinCreator {
        /** */
