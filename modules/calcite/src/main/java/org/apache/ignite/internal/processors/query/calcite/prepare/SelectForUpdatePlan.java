 ...
 * column appended at the end.  At execution time the executor:
 * <ol>
 *   <li>Runs the inner plan and materialises the full result set.</li>
 *   <li>Extracts the last column (_KEY) from every row.</li>
...
public class SelectForUpdatePlan extends AbstractQueryPlan {
    /** Inner plan: SELECT with _KEY appended as the last column. */
    private final MultiStepQueryPlan innerPlan;

    /** Number of user-visible result columns (total inner columns minus the appended _KEY). */
    ...
    /** Returns the inner plan that reads rows together with the appended {@code _KEY} column. */
    public MultiStepQueryPlan innerPlan() {
        return innerPlan;
    }

    /** Number of user-visible result columns (the _KEY column is not counted). */
