...
public class CacheTableDescriptorImpl extends NullInitializerExpressionFactory
    ...
    public CacheTableDescriptorImpl(GridCacheContextInfo<?, ?> cacheInfo, GridQueryTypeDescriptor typeDesc,
        ...
        if (typeDesc.implicitPk()) {
            ...
            descriptors.add(
                new KeyValDescriptor(QueryUtils.KEY_FIELD_NAME, typeDesc.keyClass(), null, true, QueryUtils.KEY_COL) {
            ...
            descriptors.add(new KeyValDescriptor(QueryUtils.KEY_FIELD_NAME, typeDesc.keyClass(), typeDesc.keyComponentClasses(),
                true, QueryUtils.KEY_COL));
        }

        descriptors.add(
            new KeyValDescriptor(QueryUtils.VAL_FIELD_NAME, typeDesc.valueClass(), null, false, QueryUtils.VAL_COL));
    ...
    private static class KeyValDescriptor implements CacheColumnDescriptor {
        ...
        /** */
        @Nullable private final List<Class<?>> componentTypes;

        /** */
        private volatile RelDataType logicalType;

        /** */
        private KeyValDescriptor(String name, Class<?> type, @Nullable List<Class<?>> componentTypes, boolean isKey, int fieldIdx) {
            this.name = name;
            this.isKey = isKey;
        ...
        private KeyValDescriptor(String name, GridQueryProperty desc, boolean isKey, int fieldIdx) {
            ...
            componentTypes = desc.componentTypes();
        }
