 ...
 */
@ComputeTaskName("GarDelegatingTask")
public class GarDelegatingTask extends ComputeTaskAdapter<String, String> {
        ...
        for (ClusterNode node : subgrid) {
            result.put(new ComputeJobAdapter() {
                ...
                @Override public Serializable execute() {
                    return ignite.compute(ignite.cluster().forPredicate(
                        F.equalTo(F.first(ignite.cluster().forRemotes().nodes()))))
                        .execute("LocalUseServiceTask", "Hello");
                }
            }, node);
        }
        return result;
