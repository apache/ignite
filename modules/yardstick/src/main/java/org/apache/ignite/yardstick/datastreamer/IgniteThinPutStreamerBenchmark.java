 ...
 *
 */
public class IgniteThinPutStreamerBenchmark extends IgniteThinAbstractBenchmark {
    /** Actual benchmark. */
        ...
        benchmark = new IgniteAbstractStreamerBenchmark() {
            @Override <K, V> DataStreamer<K, V> dataStreamer(String cacheName) {
                return new DataStreamer<K, V>() {
                    private final ClientCache<K, V> cache = client().cache(cacheName);
            ...
            @Override int cacheSize(String cacheName) {
                return client().cache(cacheName).size();
            }
        };

        benchmark.setUp(getClass().getSimpleName(), args, cfg, client().cacheNames());
    }
