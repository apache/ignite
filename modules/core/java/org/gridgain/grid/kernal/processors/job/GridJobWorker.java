...
public class GridJobWorker extends GridWorker implements GridTimeoutObject {
    ...
    @Override public void onTimeout() {
        ...
        if (ctx.event().isRecordable(EVT_JOB_TIMEDOUT))
    ...
    boolean initialize(GridDeployment dep, Class<?> taskCls) {
        ...
        try {
            ...
            if (ctx.event().isRecordable(EVT_JOB_QUEUED))
    ...
    @Override protected void body() {
        ...
        if (ctx.event().isRecordable(EVT_JOB_STARTED))
    ...
    public void cancel(boolean sys) {
        try {
            ...
            if (ctx.event().isRecordable(EVT_JOB_CANCELLED))
    ...
    void finishJob(@Nullable Object res, @Nullable GridException ex, boolean sndReply) {
        ...
        try {
                    ...
                    if (sndNode == null) {
                        ...
                        try {
                            if (ex != null) {
                                ...
                                else if (ctx.event().isRecordable(EVT_JOB_REJECTED))
                                    evts = addEvent(evts, EVT_JOB_REJECTED, "Job has not been started " +
                                        "[ex=" + ex + ", job=" + job + ']');
                            }
                            else if (ctx.event().isRecordable(EVT_JOB_FINISHED))
            ...
            // Job timed out.
            else if (ctx.event().isRecordable(EVT_JOB_FAILED))
    ...
    boolean onMasterNodeLeft() {
        if (job instanceof GridComputeJobMasterLeaveAware) {
            if (masterLeaveGuard.compareAndSet(false, true)) {
                try {
                    ((GridComputeJobMasterLeaveAware)job).onMasterNodeLeft(ses.session());
