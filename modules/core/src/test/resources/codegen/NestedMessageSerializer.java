...
import org.apache.ignite.internal.NestedMessage;
 ...
 */
public class NestedMessageSerializer implements MessageSerializer {
    /** */
    @Override public boolean writeTo(Message m, ByteBuffer buf, MessageWriter writer) {
        NestedMessage msg = (NestedMessage)m;
        ...
        switch (writer.state()) {
            case 0:
                if (!writer.writeByteArray(msg.payload()))
                    return false;
    ...
    @Override public boolean readFrom(Message m, ByteBuffer buf, MessageReader reader) {
        ...
        switch (reader.state()) {
            case 0:
                msg.payload(reader.readByteArray());
