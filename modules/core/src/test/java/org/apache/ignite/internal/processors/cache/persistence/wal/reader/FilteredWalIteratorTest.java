 ...
 *
 */
public class FilteredWalIteratorTest extends TestCase {
     ...
     */
    public void testShouldReturnCorrectlyFilteredRecords() throws IgniteCheckedException {
        List<Object[]> data = providedTestData();

        for (int idx = 0; idx < data.size(); idx++) {
            Object[] ar = data.get(idx);

            shouldReturnCorrectlyFilteredRecords(
                (String)ar[0],
                (WALIterator)ar[1],
                (Predicate<IgniteBiTuple<WALPointer, WALRecord>>)ar[2],
                (List<IgniteBiTuple<WALPointer, WALRecord>>)ar[3],
                idx
            );
        }
    }
     ...
     * @throws IgniteCheckedException If failed.
     */
    private void shouldReturnCorrectlyFilteredRecords(
        String nameOfCase,
        ...
        System.out.println("Executing test #" + idx + ", case=" + nameOfCase);
     ...
     */
    public static List<Object[]> providedTestData() {
        ArrayList<Object[]> res = new ArrayList<>();

        res.addAll(prepareTestCaseData("EvenFilter", r -> r.get2().type().ordinal() % 2 == 0));
    ...
    private static IgniteBiTuple<WALPointer, WALRecord> randomRecord() {
        ...
        switch (recordId) {
            ...
            case 2:
                return new IgniteBiTuple<>(ZERO_POINTER, new PageSnapshot(new FullPageId(1, 1), dummyPage(1024, 1)));
            case 3:
