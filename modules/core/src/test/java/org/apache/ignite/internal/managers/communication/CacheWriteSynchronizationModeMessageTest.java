...
public class CacheWriteSynchronizationModeMessageTest {
    /** */
    @Test
    public void testWriteSynchronizationModeCode() {
        ...
        for (CacheWriteSynchronizationMode mode : CacheWriteSynchronizationMode.values())
            assertTrue(new CacheWriteSynchronizationModeMessage(mode).code() != -1);
    }

    /** */
    @Test
    public void testWriteSynchronizationModeFromCode() {
        ...
        assertEquals("Unknown write synchronization mode code: 3", t.getMessage());
    }

    /** */
    @Test
    public void testConversionConsistency() {
        for (CacheWriteSynchronizationMode mode : F.concat(CacheWriteSynchronizationMode.values(), (CacheWriteSynchronizationMode)null)) {
            CacheWriteSynchronizationModeMessage msg = new CacheWriteSynchronizationModeMessage(mode);

            assertEquals(mode, msg.value());
