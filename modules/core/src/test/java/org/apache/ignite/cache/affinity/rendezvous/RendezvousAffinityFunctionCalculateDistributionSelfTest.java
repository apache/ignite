...
//    /** GridStringLogger */
//    private static GridStringLogger log /**= new GridStringLogger() */;
...
//        log = new GridLoggerProxyString();

        System.setProperty(IgniteSystemProperties.IGNITE_PART_DISTRIBUTION_WARN_THRESHOLD, String.valueOf(true));

        RendezvousAffinityFunction aff = new RendezvousAffinityFunction(true, 1024);

        GridTestUtils.setFieldValue(aff, "log", log);

        ignite = startGrid();

        System.out.println("!!!!!!!!!!!! logger in affinitity text is" + aff.getLog().toString() +
