...
public class IgniteDynamicCacheStartSelfTest extends GridCommonAbstractTest {
    ...
    public void testCacheRestartIsAllowedOnlyToItsInititator() throws Exception {
        ...
        try {
            kernal.context().cache().dynamicStartCachesByStoredConf(Collections.singleton(storedCacheData), true, true, false, IgniteUuid.randomUuid(), false).get();
        ...
        kernal.context().cache().dynamicStartCachesByStoredConf(Collections.singleton(storedCacheData), true, true, false, restartId, false).get();
