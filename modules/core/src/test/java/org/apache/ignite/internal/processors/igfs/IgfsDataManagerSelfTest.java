...
public class IgfsDataManagerSelfTest extends IgfsCommonAbstractTest {
    ...
    @Override protected void beforeTest() throws Exception {
        igfs = (IgfsEx)grid(0).fileSystem("igfs");
    ...
    public void testDataStoring() throws Exception {
        for (int i = 0; i < 10; i++) {
            IgfsPath path = new IgfsPath("/foo" + i);
            ...
            info = new IgfsFileInfo(info.length() + data.length - 3, info.reservedDelta(), info);

            expectsStoreFail(info, data, "Not enough space reserved to store data", fut);

            info = new IgfsFileInfo(info.length() + 3, info.reservedDelta(), info);
            ...
            os.writeCompletionFut.markWaitingLastAck();
    ...
    public void testDataStoringRemainder() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            byte[] left = mgr.storeDataBlocks(info, info.length(), remainder, remainder.length, ByteBuffer.wrap(data),
                false, range, null, fut);

            assert left.length == blockSize / 2;

            byte[] remainder2 = new byte[blockSize / 2];

            info = new IgfsFileInfo(info.length() + remainder2.length, info.reservedDelta(), info);
    ...
    private void expectsStoreFail(final IgfsFileInfo reserved, final byte[] data, @Nullable String msg,
        ...
        GridTestUtils.assertThrows(log, new Callable() {
            @Override public Object call() throws Exception {
                ...
                mgr.storeDataBlocks(reserved, reserved.length(), null, 0, ByteBuffer.wrap(data), false, range, null, fut);
