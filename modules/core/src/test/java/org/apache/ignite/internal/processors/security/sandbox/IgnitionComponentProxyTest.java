...
public class IgnitionComponentProxyTest extends AbstractSandboxTest {
    ...
    @Override protected void prepareCluster() throws Exception {
        startGrid(SRV, ALLOW_ALL_PERMISSIONS, false);
        startGrid(CLNT, ALLOW_ALL_PERMISSIONS, true);
    }
                        ...
                        () -> {
                            ...
                            return Ignition.start(
                                optimize(getConfiguration(login,
                                    new TestSecurityPluginProvider(login, "", ALLOW_ALL_PERMISSIONS, perms, globalAuth)))
                            );
                        }, acc);
                }
                catch (Exception e) {
