...
public class MetricsConfigurationTest extends GridCommonAbstractTest {
    ...
    public void testHitRateConfiguration() throws Exception {
        try (IgniteEx g = startGrid(0)) {
            ...
            bean.configureIntervalMetric(metricName, 3000);
    ...
    public void testIntervalMetricConfiguration() throws Exception {
        try (IgniteEx g = startGrid(0)) {
            ...
            String metricName = "io.discovery.MaxMsgQueueSize";

            // Empty name.
            assertThrowsWithCause(
                () -> bean.configureIntervalMetric(null, 1),
                NullPointerException.class);

            // Wrong interval value.
            assertThrowsWithCause(
                () -> bean.configureIntervalMetric(metricName, 0),
                IllegalArgumentException.class);

            assertThrowsWithCause(
                () -> bean.configureIntervalMetric(metricName, -1),
                IllegalArgumentException.class);
            ...
            bean.configureIntervalMetric(metricName, 5000);

            AbstractIntervalMetric metric = g.context().metric().find(metricName, AbstractIntervalMetric.class);
    ...
    public void testConfigRemovedOnRegistryRemove() throws Exception {
        checkOnStartAndRestart((g0, g1) -> {
            ...
            metricsBean(g0).configureIntervalMetric(metricName(TEST_REG, INTERVAL_METRIC_NAME), 1000);
        }, (g0, g1) -> {
            MetricRegistryImpl mreg = g0.context().metric().registry(TEST_REG);
