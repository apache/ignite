            ...
            for (int toIdx = 0; toIdx < 3; ++toIdx) {
                ...
                try {
                    try (Scope ignored = OperationContext.set(USR_ATTR, usrVal)) {
                        from.context().io().sendOrderedMessage(node(from, to), TOPIC_IO_TEST, usrVal, SYSTEM_POOL, 5_000, false);
                    }

                    from.context().io().sendOrderedMessage(node(from, to), TOPIC_IO_TEST, DFLT_USR, SYSTEM_POOL, 5_000, false);

                    try (Scope ignored = OperationContext.set(PTR_ATTR, ptrVal)) {
                        from.context().io().sendOrderedMessage(node(from, to), TOPIC_IO_TEST, ptrVal, SYSTEM_POOL, 5_000, false);
                    }
