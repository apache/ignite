...
public class GridServiceMetricsTest extends GridCommonAbstractTest {
    ...
    private void testServiceMetrics(int serverCnt, int clientCnt, int perClusterCnt, int perNodeCnt) throws Throwable {
        IgniteEx first = startGrids(serverCnt);

        for (int i = 0; i < clientCnt; i++)
            startClientGrid(serverCnt + i);

        first.services().deploy(serviceCfg(new TheService(), perClusterCnt, perNodeCnt));
        ...
        for (int i = 0; i < Math.max(10, serverCnt + clientCnt); i++)
            invokeCollector += callService(grids.get(i % grids.size()));

        long invokesInMetrics = 0;

        // Calculate and check invocations within the metrics.
        for (int i = 0; i < serverCnt; i++) {
            ReadOnlyMetricRegistry metrics = findMetricRegistry(grid(i).context().metric(), SRVC_NAME);
    ...
    /** */
    private static interface ExtendedTestService extends TestService {
        /** */
        String foo(String p);
    ...
    /** Trivial implementation of the test service holding methods whcich should not be measured. */
    private static class TheService implements Service, ExtendedTestService, Externalizable {
        /** */
