...
import org.apache.ignite.internal.processors.cache.persistence.snapshot.SnapshotPartitionsCheckResult;
...
public class IncrementalSnapshotCheckBeforeRestoreTest extends AbstractSnapshotSelfTest {
    ...
    public void testCheckCorrectIncrementalSnapshot() throws Exception {
        ...
        for (IgniteEx n: F.asList(grid(0), grid(GRID_CNT))) {
            for (int i = 0; i <= incSnpCnt; i++) {
                SnapshotPartitionsCheckResult res = snp(n).checkSnapshot(SNP, null, null, false, i, DFLT_CHECK_ON_RESTORE)
                    .get(getTestTimeout());
    ...
    public void testNonExistentIncrementalSnapshot() throws Exception {
        ...
        for (IgniteEx n : F.asList(grid(0), grid(GRID_CNT))) {
            SnapshotPartitionsCheckResult res = snp(n).checkSnapshot(SNP, null, null, false, 1, DFLT_CHECK_ON_RESTORE)
                .get(getTestTimeout());
    ...
    public void testIntermediateSnapshotNotFound() throws Exception {
        ...
        for (IgniteEx n : F.asList(srv, grid(GRID_CNT))) {
            SnapshotPartitionsCheckResult res = snp(n).checkSnapshot(SNP, null, null, false, 0, DFLT_CHECK_ON_RESTORE)
                .get(getTestTimeout());
