    ...
    public void testStatistics() throws Exception {
        ...
        try {
            ...
            synchronized (mux) {
                ...
                ReadOnlyMetricRegistry mreg0 = grid(0).metrics().registry(node1regName);
                ReadOnlyMetricRegistry mreg1 = grid(1).metrics().registry(node0regName);

                LongAdderMetric sentMetric = mreg0.metric(SENT_MESSAGES_BY_NODE_ID_METRIC_NAME);
                assertNotNull(sentMetric);
                assertEquals(mbean0.getSentMessagesCount(), sentMetric.value());

                LongAdderMetric rcvMetric = mreg1.metric(RECEIVED_MESSAGES_BY_NODE_ID_METRIC_NAME);
                assertNotNull(rcvMetric);
                assertEquals(mbean1.getReceivedMessagesCount(), rcvMetric.value());

                stopGrid(1);

                mreg0 = grid(0).metrics().registry(node1regName);

                sentMetric = mreg0.metric(SENT_MESSAGES_BY_NODE_ID_METRIC_NAME);
