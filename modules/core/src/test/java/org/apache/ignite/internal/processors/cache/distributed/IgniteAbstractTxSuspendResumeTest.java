...
public abstract class IgniteAbstractTxSuspendResumeTest extends GridCommonAbstractTest {
        ...
        executeTestForAllCaches(new CI2Exc<Ignite, IgniteCache<Integer, Integer>>() {
            @Override public void applyx(Ignite ignite, final IgniteCache<Integer, Integer> cache) throws Exception {
                for (TransactionIsolation isolation : TransactionIsolation.values()) {
                    ...
                    final Transaction tx = ignite.transactions().txStart(transactionConcurrency(), isolation);
                    System.out.println("[txs] Test 1");
                    final AtomicInteger cntr = new AtomicInteger(0);

                    cache.put(-1, -1);
                    otherCache.put(-1, -1);
                    System.out.println("[txs] Test 2");

                    tx.suspend();
                    System.out.println("[txs] Test 3");
                    ...
                    System.out.println("[txs] Test 4");

                    tx.resume();
                    System.out.println("[txs] Test 5");

                    cache.remove(-1);
                    otherCache.remove(-1);
                    System.out.println("[txs] Test 6");

                    tx.commit();
                    System.out.println("[txs] Test 7");
                    ...
                    System.out.println("[txs] Test 8");

                    assertFalse(cache.containsKey(-1));
                    assertFalse(otherCache.containsKey(-1));
                    System.out.println("[txs] Test 9");

                    cache.removeAll();
                    otherCache.removeAll();
                    System.out.println("[txs] Test 10");
    ...
    public void testTxTimeoutOnResumed() throws Exception {
        executeTestForAllCaches(new CI2Exc<Ignite, IgniteCache<Integer, Integer>>() {
            @Override public void applyx(Ignite ignite, final IgniteCache<Integer, Integer> cache) throws Exception {
                for (TransactionIsolation isolation : TransactionIsolation.values()) {
                    final Transaction tx = ignite.transactions().txStart(transactionConcurrency(), isolation, TX_TIMEOUT, 0);
...
//    public void testCacheLockClashesWithTx() throws Exception {
//        executeTestForAllCaches(new CI2Exc<Ignite, IgniteCache<Integer, Integer>>() {
//            @Override public void applyx(Ignite ignite, final IgniteCache<Integer, Integer> cache) throws Exception {
//                for (TransactionIsolation isolation : TransactionIsolation.values()) {
//                    Lock lock = null;
...
//
//                    try {
//                        tx = ignite.transactions().txStart(transactionConcurrency(), isolation);
//
//                        cache.put(1, 1);
//
//                        tx.suspend();
