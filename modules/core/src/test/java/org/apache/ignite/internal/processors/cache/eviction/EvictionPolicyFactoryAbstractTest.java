...
public abstract class EvictionPolicyFactoryAbstractTest<T extends EvictionPolicy<?, ?>>
    ...
    /** @return Policy. */
    protected T policy() {
        GridCacheEvictionManager evictMgr = grid().cachex(DEFAULT_CACHE_NAME).context().evicts();

        assert evictMgr != null;

        return (T)evictMgr.getEvictionPolicy();
    }
    ...
    protected T policy(int i) {
        GridCacheEvictionManager evictMgr = grid(i).cachex(DEFAULT_CACHE_NAME).context().evicts();
    ...
    protected T nearPolicy(int i) {
        GridCacheEvictionManager evictMgr = grid(i).cachex(DEFAULT_CACHE_NAME).context().near().context().evicts();
