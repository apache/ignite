...
class DiscoveryTcpSpiDataExchangeImpl implements DiscoveryTcpSpiDataExchange {
    ...
    /** {@inheritDoc} */
    @Override public Map<Integer, byte[]> collectClientPrefetchData() {
        Map<Integer, byte[]> prefetchData = new HashMap<>();
        ...
        for (GridComponent c : ctx.components()) {
            ...
            Serializable data = c.collectTcpClientPrefetchData();

            if (data != null) {
                try {
                    byte[] componentDataBytes = marshaller.marshal(data);

                    prefetchData.put(dataExchangeType.ordinal(), componentDataBytes);
                }
                catch (IgniteCheckedException e) {
                    log.error("Cannot marshal handshake response data", e);
                }
    ...
    /** {@inheritDoc} */
    @Override public void clientPrefetchDataReceived(Map<Integer, byte[]> prefetchData) {
        if (prefetchData == null)
            prefetchData = Collections.emptyMap();
        ...
        for (GridComponent c : ctx.components()) {
            ...
            byte[] componentDataBytes = prefetchData.get(dataExchangeType.ordinal());

            Serializable data = null;

            try {
                data = componentDataBytes == null
                    ? null
                    : marshaller.unmarshal(componentDataBytes, U.gridClassLoader());
            }
            catch (IgniteCheckedException e) {
            ...
            c.onTcpClientPrefetchDataReceived(data);
        }
    }
}
