...
public class GridCacheCommandHandler extends GridRestCommandHandlerAdapter {
    ...
    @Override public IgniteInternalFuture<GridRestResponse> handleAsync(final GridRestRequest req) {
        ...
        try {
            ...
            switch (cmd) {
                case CACHE_GET: {
                    fut = executeCommand(req.destinationId(), req.clientId(), cacheName, skip, key,
                        new GetCommand(key));
                ...
                case CACHE_PUT: {
                    ...
                    fut = executeCommand(req.destinationId(), req.clientId(), cacheName, skip, key, new
    ...
    private IgniteInternalFuture<GridRestResponse> executeCommand(
        ...
        if (locExec) {
            CacheProjection<?,?> prj = localCache(cacheName).forSubjectId(clientId).setSkipStore(skipStore);
    ...
    private static class FlaggedCacheOperationCallable implements Callable<GridRestResponse>, Serializable {
        ...
        @Override public GridRestResponse call() throws Exception {
            CacheProjection<?, ?> prj = cache(g, cacheName).forSubjectId(clientId).setSkipStore(skipStore);
