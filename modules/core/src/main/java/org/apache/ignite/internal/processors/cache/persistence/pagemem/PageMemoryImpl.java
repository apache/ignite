...
public class PageMemoryImpl implements PageMemoryEx {
     ...
     * @param sizes data segment's (chunk) sizes, last is checkpoint pool size.
    ...
    @Override public long allocatePage(int cacheId, int partId, byte flags) throws IgniteCheckedException {
        ...
        try {
            ...
            seg.loadedPages.put(cacheId, PageIdUtils.effectivePageId(pageId), relPtr, seg.partGeneration(cacheId, partId));
        }
    ...
    private class Segment extends ReentrantReadWriteLock {
        ...
        /** Maps partition (cacheId, partId) to its generation. Generation is 1-based incrementing partition counter. */
