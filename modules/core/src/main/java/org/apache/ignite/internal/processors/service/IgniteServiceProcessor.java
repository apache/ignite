...
public class IgniteServiceProcessor extends ServiceProcessorAdapter implements IgniteChangeGlobalStateSupport {
    ...
    private void cancelDeployedServices() {
        ...
        ctx.metric().remove(ServiceContextImpl.SERVICE_METRIC_REGISTRY);
    }
    ...
    void redeploy(IgniteUuid srvcId, ServiceConfiguration cfg,
        ...
        for (final ServiceContextImpl srvcCtx : toInit) {
            ...
            try {
                ...
                srvcCtx.service(srvc, ctx.metric());
            }
            catch (Throwable e) {
    ...
    /**
     * Cancels and removes given number of service instances from the collection. If entyre collection is canceled,
     * removes the service metrics.
     *
    ...
    private void cancel(Collection<ServiceContextImpl> ctxs, int cancelCnt) {
        ServiceContextImpl first = null;

        for (Iterator<ServiceContextImpl> it = ctxs.iterator(); it.hasNext(); ) {
            cancel(first == null ? (first = it.next()) : it.next());
