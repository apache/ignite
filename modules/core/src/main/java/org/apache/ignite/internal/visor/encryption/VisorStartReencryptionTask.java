...
/**
 * The task to start re-encryption of the specified cache group.
 */
public class VisorStartReencryptionTask extends VisorMultiNodeTask<Integer, Map<UUID, Object>, Boolean> {
    /** Serial version uid. */
    ...
    private static class VisorStartReencryptionJob extends VisorJob<Integer, Boolean> {
        ...
        @Override protected Boolean run(Integer grpId) throws IgniteException {
            try {
                if (!ignite.context().encryption().reencryptionFuture(grpId).isDone())
                    return false;
