...
public class GridCacheProjectionImpl<K, V> implements GridCacheProjectionEx<K, V>, Externalizable {
    ...
    public GridCacheProjectionImpl(
        ...
        @Nullable IgnitePredicate<? super Entry<K, V>> entryFilter,
     ...
     */
    IgnitePredicate<Entry<K, V>> entryFilter(boolean noNulls) {
     ...
     */
    IgnitePredicate<Entry<K, V>> and(
        IgnitePredicate<Entry<K, V>> filter, boolean noNulls) {
        IgnitePredicate<Entry<K, V>> entryFilter = entryFilter(noNulls);
     ...
     */
    private IgnitePredicate<Entry<K, V>> and(@Nullable final IgnitePredicate<Entry<K, V>>[] f1,
        boolean nonNulls) {
        IgnitePredicate<Entry<K, V>> entryFilter = entryFilter(nonNulls);
     ...
     */
    boolean isAll(Entry<K, V> e, boolean noNulls) {
     ...
     */
    private IgniteInClosure<Entry<K, V>> visitor(final IgniteInClosure<Entry<K, V>> vis) {
        return new CI1<Entry<K, V>>() {
            @Override public void apply(Entry<K, V> e) {
                if (isAll(e, true))
                    vis.apply(e);
            }
        };
     ...
     * @return Projection-filter-aware visitor.
     */
    private IgnitePredicate<Entry<K, V>> visitor(final IgnitePredicate<Entry<K, V>> vis) {
        return new P1<Entry<K, V>>() {
    ...
    @Override public <K1, V1> CacheProjection<K1, V1> projection(
        ...
        GridCacheProjectionImpl<K1, V1> prj = new GridCacheProjectionImpl<>(
            ...
            (IgnitePredicate<Entry>)noNullEntryFilter.entryFilter,
            flags,
            subjId,
            keepPortable,
            expiryPlc);
    ...
    @Override public IgniteInternalFuture<Boolean> putxAsync(K key, V val, @Nullable GridCacheEntryEx<K, V> entry,
        long ttl, @Nullable IgnitePredicate<Entry<K, V>>[] filter) {
        A.notNull(key, "key", val, "val");
    ...
    @Override public IgniteInternalFuture<Boolean> replaceAsync(K key, V oldVal, V newVal) {
        IgnitePredicate<Entry<K, V>> fltr = and(F.<K, V>cacheContainsPeek(oldVal), false);
     ...
     * @param <V> Value type.
     */
    public static class FullFilter<K, V> implements IgnitePredicate<Entry<K, V>> {
        /** */
        private static final long serialVersionUID = 0L;
        ...
        /** Entry filter. */
        private IgnitePredicate<? super Entry<K, V>> entryFilter;

        /**
         * @param kvFilter Key-value filter.
         * @param entryFilter Entry filter.
         */
        private FullFilter(KeyValueFilter<K, V> kvFilter, IgnitePredicate<? super Entry<K, V>> entryFilter) {
         ...
         * @return Entry filter.
         */
        public IgnitePredicate<? super Entry<K, V>> entryFilter() {
            return entryFilter;
        }
