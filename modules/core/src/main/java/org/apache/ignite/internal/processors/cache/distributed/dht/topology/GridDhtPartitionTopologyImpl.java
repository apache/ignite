...
public class GridDhtPartitionTopologyImpl implements GridDhtPartitionTopology {
        ...
        try {
            ...
            try {
                ...
                if (exchangeVer != null) {
                    ...
                    if (lostParts != null)
                        applyLostPartitionsFromFullMessage(exchFut, lostParts);
                }
    ...
    @Override public void onExchangeDone(@Nullable GridDhtPartitionsExchangeFuture fut,
        ...
        try {
            assert assignment.topologyVersion().equals(topReadyFut.context().events().topologyVersion());
                    ...
                    if (lost) {
                        ...
                        for (Map.Entry<UUID, GridDhtPartitionMap> entry : node2part.entrySet()) {
                            ...
                            if (p0 != null && p0 != EVICTED)
                                entry.getValue().put(part, isLossIgnored ? OWNING : LOST);
                        }
                    }
                }
    ...
    @Override public Set<Integer> lostPartitions() {
        ...
        try {
            return lostParts == null ? Collections.emptySet() : new HashSet<>(lostParts);
        }
        finally {
            lock.readLock().unlock();
        }
    ...
    /** */
    private void applyLostPartitionsFromFullMessage(GridDhtPartitionsExchangeFuture fut, Set<Integer> lostParts) {
        assert lock.isWriteLockedByCurrentThread();

        Set<Integer> recentlyLost = new HashSet<>();

        for (Integer part : lostParts) {
            if (!F.contains(this.lostParts, part))
                recentlyLost.add(part);
            ...
            if (locPart != null && locPart.state() != EVICTED) {
                ...
                GridDhtPartitionMap locMap = node2part.get(ctx.localNodeId());
        ...
        recordPartitionLossEvents(fut, lostParts, isParitionLossIgnored());
    }

    /** */
    private void restoreLostPartitions(CacheGroupRecoveryState grpState) {
        Set<Integer> recoveryLostParts = grpState.lostParititons();

        recoveryLostParts.forEach(this::addLostPartition);

        if (!F.isEmpty(lostParts)) {
            U.warn(log, "Partitions were restored in LOST state during cluster activation [partIds=" +
                S.toStringSortedDistinct(lostParts) + ']');
        }
    }
    ...
    private boolean isParitionLossIgnored() {
        ...
        return isIgnorePlcSupported && grp.config().getPartitionLossPolicy() == IGNORE;
    }

    /** */
    private void recordPartitionLossEvents(GridDhtPartitionsExchangeFuture fut, Collection<Integer> recentlyLost, boolean isLossIgnored) {
        if (F.isEmpty(recentlyLost))
            return;

        if (fut != null && grp.eventRecordable(EVT_CACHE_REBALANCE_PART_DATA_LOST)) {
        ...
        U.warn(log, "Detected lost partitions" + (isLossIgnored ? " (will ignore)" : "")
            + " [grp=" + grp.cacheOrGroupName()
    ...
    /** */
    private boolean isDeactivationInProgress(GridDhtPartitionsExchangeFuture exchFut) {
        if (exchFut == null)
            return false;
