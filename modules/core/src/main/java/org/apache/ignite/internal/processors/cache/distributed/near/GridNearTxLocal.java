...
public class GridNearTxLocal extends GridDhtTxLocalAdapter implements GridTimeoutObject, AutoCloseable {
    ...
    private <K, V> IgniteInternalFuture putAsync0(
        ...
        try {
            ...
            CacheReturnMode cacheReturnMode = cacheCtx.cacheReturnMode();
            ...
            if (pessimistic()) {
                ...
                return optimisticPutFuture(cacheCtx, loadFut, ret, cacheReturnMode);
        }
        catch (IgniteCheckedException e) {
            return new GridFinishedFuture(e);
        }
        ...
        if (txEntry == null) {
            while (true) {
                ...
                try {
                    ...
                    // Pessimistic.
                        ...
                        if (retval && !transform)
                            ret.set(cacheCtx, old, true, cacheReturnMode, U.deploymentClassLoader(cctx.kernalContext(), deploymentLdrId));
                        else
                            ret.success(true);
                    }
            ...
            if (!del) {
                if (hasFilters && !filter(entry.context(), cacheKey, v, filter)) {
                    ret.set(cacheCtx, v, false, cacheReturnMode, U.deploymentClassLoader(cctx.kernalContext(), deploymentLdrId));
            ...
            if (!pessimistic()) {
                ...
                if (retval && !transform)
                    ret.set(cacheCtx, v, true, cacheReturnMode, U.deploymentClassLoader(cctx.kernalContext(), deploymentLdrId));
                else
                    ret.success(true);
            }
        }
    ...
    private <K, V> IgniteInternalFuture<GridCacheReturn> removeAllAsync0(
        ...
        final CacheReturnMode cacheReturnMode = cacheCtx.cacheReturnMode();
                ...
                PLC2<Map<K, V>> plc2 = new PLC2<Map<K, V>>() {
                    @Override public IgniteInternalFuture<Map<K, V>> postLock() throws IgniteCheckedException {
                        ...
                        for (KeyCacheObject cacheKey : lockKeys) {
                            K keyVal = (K)cacheCtx.cacheObjectContext().unwrapBinaryIfNeeded(cacheKey, cacheReturnMode, true, null);
                                        ...
                                        try {
                                            ...
                                            for (Map.Entry<KeyCacheObject, EntryGetResult> entry : fut.get().entrySet()) {
                                                ...
                                                if (getRes != null)
                                                    ...
                                                    retMap.remove((K)cacheCtx.cacheObjectContext().unwrapBinaryIfNeeded(
                                                        key,
                                                        cacheReturnMode,
                                                        false,
                                                        U.deploymentClassLoader(cctx.kernalContext(), deploymentLdrId)));
                ...
                if (!missed.isEmpty()) {
                    if (!readCommitted())
                        for (Iterator<KeyCacheObject> it = missed.keySet().iterator(); it.hasNext(); ) {
                            ...
                            K keyVal = (K)cacheCtx.cacheObjectContext().unwrapBinaryIfNeeded(
                                cacheKey,
                                cacheReturnMode,
                                false,
                                null
                                ...
                                if (needReadVer) {
                                    getRes = primaryLocal(entry) ?
                                        entry.innerGetVersioned(
                                            ...
                                            resolveTaskName(),
                                            accessPlc,
                                            cacheReturnMode != CacheReturnMode.DESERIALIZED,
                                            null) : null;
        ...
        GridInClosure3<KeyCacheObject, Object, GridCacheVersion> c =
            (KeyCacheObject key, @Nullable Object val, @Nullable GridCacheVersion loadVer) -> {
                ...
                if (singleRmv) {
                    ...
                    ret.set(cacheCtx, null, val != null, cacheReturnMode, U.deploymentClassLoader(cctx.kernalContext(), deploymentLdrId));
                }
                else {
                    ...
                    @Override public GridCacheReturn applyx(IgniteInternalFuture<IgniteInternalTx> txFut)
                        ...
                        try {
                            ...
                            if (implicitRes.invokeResult()) {
                                ...
                                res = cacheCtx.unwrapInvokeResult((Map)res, cacheReturnMode != DESERIALIZED);
                            }
    ...
    public IgniteInternalFuture<IgniteInternalTx> commitNearTxLocalAsync() {
        ...
        if (!fastFinish) {
            ...
            prepareFut.listen(f -> {
                ...
                if (!onePhaseCommit)
                    incrementalSnapshotId(cctx.snapshotMgr().incrementalSnapshotId());
    ...
    private <K, V> IgniteInternalFuture<Map<K, V>> checkMissed(
        ...
        return new GridEmbeddedFuture<>((Void v, Exception e) -> {
            ...
            loadMissing(
                ...
                readRepairStrategy,
                expiryPlc,
                (KeyCacheObject key, Object val, GridCacheVersion loadVer) -> {
                    CacheObject cacheVal = cacheCtx.toCacheObject(val);
