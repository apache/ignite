...
public class GridTimeoutProcessor extends GridProcessorAdapter {
     ...
     *
     */
    public class SoftHardTimeoutTask {
        /** Soft timeout millis. */
        ...
        /** Soft timeout object. */
        private final AtomicReference<SoftHardTimeoutObject> softTimeoutObj = new AtomicReference<>();

        /** Hard timeout object. */
        private final AtomicReference<SoftHardTimeoutObject> hardTimeoutObj = new AtomicReference<>();

        /** Active. */
        private volatile boolean active;

        /**
         * @param softTimeoutMillis Soft timeout millis.
         * @param task Task.
         */
        public SoftHardTimeoutTask(long softTimeoutMillis, Runnable task) {
            this.softTimeoutMillis = softTimeoutMillis;
            this.task = task;
        }

        /**
         * @param softTimeoutObj Soft timeout object.
         * @param hardTimeoutObj Hard timeout object.
         */
        void init(SoftHardTimeoutObject softTimeoutObj, SoftHardTimeoutObject hardTimeoutObj) {
            assert softTimeoutObj.task == this;
            assert hardTimeoutObj.task == this;

            this.softTimeoutObj.set(softTimeoutObj);
            this.hardTimeoutObj.set(hardTimeoutObj);
        }

        /**
         *
         */
        public boolean resetSoftTimeout() {
            SoftHardTimeoutObject newSoftTimeoutObj;

            synchronized (this) {
                if (!active)
                    return false;

                newSoftTimeoutObj = new SoftHardTimeoutObject(this, System.currentTimeMillis() + softTimeoutMillis);

                softTimeoutObj.set(newSoftTimeoutObj);
            }

            // TODO : handle 'false' case.
            return addTimeoutObject(newSoftTimeoutObj);
        }

        /**
         * @param timeoutId Timeout id.
         */
        void onTimeout(IgniteUuid timeoutId) {
            synchronized (this) {
                if (!active)
                    return;

                if (!hardTimeoutObj.get().timeoutId().equals(timeoutId) && !softTimeoutObj.get().timeoutId().equals(timeoutId))
                    return;
     ...
     *
     */
    public class SoftHardTimeoutObject implements GridTimeoutObject {

        /** Timeout id. */
        private final IgniteUuid timeoutId = IgniteUuid.randomUuid();

        /** Task. */
        private final SoftHardTimeoutTask task;
         ...
         */
        public SoftHardTimeoutObject(SoftHardTimeoutTask task, long endTime) {
            this.task = task;
