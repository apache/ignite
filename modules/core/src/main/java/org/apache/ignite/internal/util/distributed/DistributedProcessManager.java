...
import org.apache.ignite.internal.managers.encryption.MasterKeyChangeResult;
 ...
 * @param <F> Type of action message.
 */
public class DistributedProcessManager<I extends InitMessage,
    R extends SingleNodeMessage, F extends FinishMessage> {
    /** Map of all active processes. */
    private final ConcurrentHashMap</*processId*/UUID, Process> processes = new ConcurrentHashMap<>(1);

    private DistributedProcess<I, R, F> acts;
     ...
     * @param initCls Class of the initial message {@link InitMessage}.
     * @param resCls  Class of the single nodes result message {@link SingleNodeMessage}.
     * @param topic   Grid topic of result message.
     * @param actCls  Class of the action message {@link FinishMessage}.
     */
    public DistributedProcessManager(GridKernalContext ctx, Class<I> initCls, Class<R> resCls, GridTopic topic, Class<F> actCls,
        DistributedProcess<I, R, F> acts) {
        this.ctx = ctx;
        this.acts = acts;
        ...
        ctx.discovery().setCustomEventListener(initCls, (topVer, snd, msg) -> {
            ...
            IgniteInternalFuture<R> fut = acts.execute(msg);
    ...
    protected void onAllReceived(Map<UUID, R> res) {
        F msg = acts.buildFinishMessage(res);
