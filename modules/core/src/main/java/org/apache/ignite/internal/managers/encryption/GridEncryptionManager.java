        ...
        if (dataBag.isJoiningNodeClient() && !ctx.clientNode() && joiningNodeKeys.newKeys != null) {
            ...
            for (Map.Entry<Integer, byte[]> e : joiningNodeKeys.newKeys.entrySet()) {
                ...
                if (dataBag.commonDataCollectedFor(ENCRYPTION_MGR.ordinal())) {
                    U.quietAndInfo(log, "Storing the new encryption key for the cache group " +
                        "configured on the client node [grp=" + grpId + ']');

                    encKey = commonData.get(grpId);
                }
                else {
                    U.quietAndInfo(log, "Generating and storing new encryption key for the cache group " +
                        "configured on the client node [grp=" + grpId + ']');
