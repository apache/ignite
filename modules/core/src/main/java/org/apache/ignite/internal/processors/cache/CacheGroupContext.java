...
public class CacheGroupContext {
    ...
    public long restorePartitionStates(Map<GroupPartitionId, PartitionRecoverState> partitionRecoveryStates) throws IgniteCheckedException {
        ...
        // TODO: https://ggsystems.atlassian.net/browse/TDR-113
        // Temporary fix for 'MOVING' partitions after snapshot `RESTORE`.
        if (offheap() instanceof GridCacheOffheapManager) {
            GridCacheOffheapManager offheap = (GridCacheOffheapManager)offheap();

            if (offheap.needToRegister())
                ((GridCacheDatabaseSharedManager)ctx.database()).addCheckpointListener(offheap);
        }
