...
/**
 * This class defines command-line Ignite Dump Reader startup. This startup can be used to start Ignite
 * Dump Reader application outside of any hosting environment from command line.
 ...
 * <p>
 * Note that scripts {@code ${IGNITE_HOME}/bin/ignite-dump-reader.{sh|bat}} shipped with Ignite use
 * this startup and you can use them as an example.
 * <p>
 *
 * @see DumpReaderMain
 */
public class DumpReaderCommandLineStartup {
    /** Quite log flag. */
    ...
    public static void main(String[] args) {
        if (!QUITE) {
            ...
            X.println("Ignite Dump Reader Command Line Startup, ver. " + ACK_VER_STR);
            X.println(COPYRIGHT);
            X.println();
        }
    ...
    private static void exit(@Nullable String errMsg, boolean showUsage, int exitCode) {
        ...
        if (showUsage) {
            X.error(
                "Usage:",
                "    ignite-dump-reader.{sh|bat} [?]|[path]",
                "    Where:",
