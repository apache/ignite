 ...
 * <br>
 * Result: {@link CacheIdleVerifyTaskResultV2} with conflict partitions.
 * <br>
 * Works properly only on idle cluster - there may be false positive conflict reports if data in cluster is being
 * concurrently updated.
 */
@GridInternal
public class VerifyBackupPartitionsTaskV2 extends ComputeTaskAdapter<CacheIdleVerifyCommandArg, CacheIdleVerifyTaskResultV2> {
    ...
    /** {@inheritDoc} */
    @Nullable @Override public CacheIdleVerifyTaskResultV2 reduce(List<ComputeJobResult> results) throws IgniteException {
     ...
     */
    public static CacheIdleVerifyTaskResultV2 reduce0(List<ComputeJobResult> results) {
        ...
        if (results.size() != ex.size())
            return new CacheIdleVerifyTaskResultV2(clusterHashes, ex);
        else
            return new CacheIdleVerifyTaskResultV2(ex);
    }
