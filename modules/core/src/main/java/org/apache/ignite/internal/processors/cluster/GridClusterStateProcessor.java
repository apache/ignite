...
public class GridClusterStateProcessor extends GridProcessorAdapter implements IGridClusterStateProcessor, MetastorageLifecycleListener {
        ...
        if (!dataBag.commonDataCollectedFor(STATE_PROC.ordinal())) {
            ...
            if (joiningNodeData != null) {
                ...
                try {
                    ...
                    U.error(log, "Failed to unmarshal discovery data from joining node: " + joiningNodeData.joiningNodeId());
    ...
    private void sendComputeChangeGlobalState(
        ...
        try {
            marshalledBlt = MARSH.marshal(blt);
        }
        catch (IgniteCheckedException e) {
            U.error(log, "Failed to marshall BaselineTopology, stateChange request won't be sent", e);

            return;
        }

        IgniteFuture<Void> fut = comp.runAsync(new ClientChangeGlobalStateComputeRequest(activate, marshalledBlt, forceBlt));
    ...
    private static class ClientChangeGlobalStateComputeRequest implements IgniteRunnable {
        ...
        /** */
        private final byte[] marshalledBlt;
         ...
         * @param activate New cluster state.
         */
        private ClientChangeGlobalStateComputeRequest(boolean activate, byte[] blt, boolean forceBlt) {
            this.activate = activate;
            this.marshalledBlt = blt;
        ...
        @Override public void run() {
            try {
                ...
                if (marshalledBlt != null)
                    blt = MARSH.unmarshal(marshalledBlt, U.resolveClassLoader(ig.configuration()));

                ig.context().state().changeGlobalState(
                    activate,
                    blt != null ? blt.currentBaseline() : null,
                    forceChangeBaselineTopology
                ).get();
            }
            catch (IgniteCheckedException ex) {
