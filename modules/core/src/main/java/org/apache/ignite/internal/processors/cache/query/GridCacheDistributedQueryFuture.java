...
public class GridCacheDistributedQueryFuture<K, V, R> extends GridCacheQueryFutureAdapter<K, V, R> {
    ...
    /** */
    private final PagesCollector pagesCollector;
    ...
    protected GridCacheDistributedQueryFuture(GridCacheContext<K, V> ctx, long reqId, GridCacheQueryBean qry,
        ...
        pagesCollector = new PagesCollector(nodes);

        reducer = qry.query().type() == TEXT ?
            new MergeSortDistributedCacheQueryReducer<R>(pagesCollector::nextNodePage, textResultComparator, nodes)
            : new UnsortedDistributedCacheQueryReducer<>(pagesCollector::nextNodePage, nodes);
    }

    /** {@inheritDoc} */
    @Override protected void cancelQuery() {
        pagesCollector.cancel();
    ...
    @Override protected void onNodeLeft(UUID nodeId) {
        boolean qryNode = pagesCollector.remoteQueryNode(nodeId);
    ...
    private class PagesCollector {
         ...
         */
        public PagesCollector(Collection<ClusterNode> nodes) {
