...
public class GridDhtPartitionTopologyImpl implements GridDhtPartitionTopology {
    ...
    private boolean checkEvictions(long updateSeq, AffinityAssignment aff) {
        ...
        List<GridDhtLocalPartition> rentingQueue = new ArrayList<>();
            ...
            if (state.active()) {
                ...
                if (!affNodes.contains(ctx.localNode())) {
                    ...
                    if (nodeIds.containsAll(F.nodeIds(affNodes))) {
                        ...
                        updateSeq = updateLocal(part.id(), RENTING, updateSeq, aff.topologyVersion());

                        changed = true;

                        rentingQueue.add(part);
        ...
        rentingQueue.forEach(part -> part.rent(true));
