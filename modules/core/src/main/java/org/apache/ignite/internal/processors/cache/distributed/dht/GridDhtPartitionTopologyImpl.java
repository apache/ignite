...
class GridDhtPartitionTopologyImpl implements GridDhtPartitionTopology {
    ...
    private List<ClusterNode> nodes(int p,
        ...
        for (ClusterNode node : partNodes) {
            if (topVer.topologyVersion() > 0 && !F.contains(allIds, node.id()))
                continue;
    ...
    @Override public boolean detectLostPartitions(DiscoveryEvent discoEvt) {
        ...
        try {
            ...
            for (int p = 0; p < parts; p++) {
                ...
                List<ClusterNode> nodes = part2node[p];

                if (nodes != null) {
                    for (ClusterNode node : nodes) {
                        GridDhtPartitionMap partMap = node2part.get(node.id());
            ...
            if (lost != null) {
                ...
                for (Integer part : lost) {
                    ...
                    // Update map for remote node.
                        ...
                        List<ClusterNode> nodes = part2node[part];

                        if (nodes != null) {
                            for (ClusterNode node : nodes) {
                                GridDhtPartitionMap nodeMap = node2part.get(node.id());
        ...
        try {
            ...
            for (int part = 0; part < parts; part++) {
                ...
                if (nodes != null) {
                    ...
                    for (ClusterNode node : nodes) {
                        GridDhtPartitionMap map = node2part.get(node.id());
        ...
        if (CONSISTENCY_CHECK) {
            ...
            for (int p = 0; p < part2node0.length; p++) {
                ...
                for (ClusterNode node : partNodes) {
                    ...
                    assert map != null : "Failed consistency check [part=" + p + ", nodeId=" + node.id() + ']';
                    assert map.containsKey(p) : "Failed consistency check [part=" + p +
                        ", nodeId=" + node.id() + ']';
                }
            }
        }
