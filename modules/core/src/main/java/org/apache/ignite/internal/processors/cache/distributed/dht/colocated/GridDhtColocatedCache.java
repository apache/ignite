...
public class GridDhtColocatedCache<K, V> extends GridDhtTransactionalCacheAdapter<K, V> {
    ...
    private void processLockResponse(UUID nodeId, GridNearLockResponse res) {
        txLockMsgLog.info("MY_DEBUG Received near lock response [txId=" + res.version() + ", node=" + nodeId + ']');
        ...
        if (fut != null)
            ...
            txLockMsgLog.info("MY_DEBUG Received near lock response for unknown future [txId=" + res.version() +
                ", node=" + nodeId +
                ", res=" + res + ']');
        }
    }
