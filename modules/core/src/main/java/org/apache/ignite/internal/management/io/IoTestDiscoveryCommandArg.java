...
public class IoTestDiscoveryCommandArg extends IgniteDataTransferObject {
    ...
    /** */
    @Order(0)
    @Argument(optional = true, description = "Number of samples (max 100, 10 by default)")
    int samples = 10;

    /** */
    @Order(1)
    @Argument(optional = true, description = "Interval between samples (millis, 10 to 60000, 100 by default)")
    long interval = 100;

    /** */
    @Order(2)
    @Argument(optional = true, description = "Payload size (bytes, max 64 KiB, 100 by default)")
    int payloadSize = 100;
