...
public class GridCacheMvccManager extends GridCacheSharedManagerAdapter {
     ...
     */
    private void cancelFuturesWithException(IgniteCheckedException err, Collection<? extends IgniteInternalFuture<?>> futures) {
        for (IgniteInternalFuture<?> fut : futures)
            try {
                ((GridFutureAdapter)fut).onDone(err);
            }
            catch (Exception e){
                U.warn(log, "Exception thrown while future cancelling", e);
            }
    }
