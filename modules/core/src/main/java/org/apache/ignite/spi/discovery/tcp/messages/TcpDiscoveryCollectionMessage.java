...
public class TcpDiscoveryCollectionMessage implements TcpDiscoveryMarshallableMessage {
    ...
    @Override public void prepareMarshal(Marshaller marsh) {
        ...
        if (F.isEmpty(writableMsgs))
            return;

        for (Message m : writableMsgs.values()) {
            if (m instanceof TcpDiscoveryMarshallableMessage)
                ((TcpDiscoveryMarshallableMessage)m).prepareMarshal(marsh);
        }
    }
    ...
    @Override public void finishUnmarshal(Marshaller marsh, ClassLoader clsLdr) {
        ...
        for (Message m : writableMsgs.values()) {
            if (m instanceof TcpDiscoveryMarshallableMessage)
                ((TcpDiscoveryMarshallableMessage)m).finishUnmarshal(marsh, clsLdr);
        }
    }
