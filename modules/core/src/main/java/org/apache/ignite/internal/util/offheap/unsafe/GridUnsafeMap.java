...
//                            bin.add(F.t(Entry.readKeyBytes(entryAddr, mem), Entry.readValueBytes(entryAddr, mem)));
                            {
                                ...
                                byte[] valBytes =  mem.readBytes(entryAddr + HEADER_SIZE + align(keyLen), valLen);
...
//                            bin.add(F.t(Entry.readKeyBytes(entryAddr, mem), Entry.readValueBytes(entryAddr, mem)));
                            {
                                ...
                                T2<Long, Integer> valPtr = new T2<>(entryAddr + HEADER_SIZE + align(keyLen), valLen);
                    ...
                    if (first != 0) {
                        ...
                        if (cur != 0) {
                            ...
                            if (evictLsnr != null) {
                                ...
                                valBytes = mem.readBytes(cur + HEADER_SIZE + align(keyLen), valLen);
                            }
        ...
        void insert(int hash, byte[] keyBytes, byte[] valBytes) {
            ...
            int size = HEADER_SIZE + align(keyBytes.length) + align(valBytes.length);
        ...
        boolean put(int hash, byte[] keyBytes, byte[] valBytes) {
            ...
            try {
                ...
                size = HEADER_SIZE + align(keyBytes.length) + align(valBytes.length);
                    ...
                    while (true) {
                        ...
                        if (Entry.keyEquals(cur, keyBytes, mem)) {
                            ...
                            if (p != null) {
                                ...
                                long valPtr = cur + HEADER_SIZE + align(keyLen);
            ...
            try {
                ...
                while (addr != 0) {
                    if (Entry.keyEquals(addr, keyBytes, mem)) {
                        ...
                        return new IgniteBiTuple<>(addr + HEADER_SIZE + align(keyLen), valLen);
                    }
...
//                        return Entry.readValueBytes(addr, mem);
                        {
                            ...
                            return mem.readBytes(addr + HEADER_SIZE + align(keyLen), valLen);
                        }
                    }
    ...
    private static class Entry {
        ...
        static int size(byte[] keyBytes, byte[] valBytes) {
            return HEADER_SIZE + align(keyBytes.length) + align(valBytes.length);
        }
        ...
        static int size(long addr, GridUnsafeMemory mem) {
            return HEADER_SIZE + align(readKeyLength(addr, mem)) + align(readValueLength(addr, mem));
        }
        ...
        static byte[] readValueBytes(long ptr, GridUnsafeMemory mem) {
            ...
            return mem.readBytes(ptr + HEADER_SIZE + align(keyLen), valLen);
        }
        ...
        static void writeValueBytes(long ptr, int keyLen, byte[] valBytes, GridUnsafeMemory mem) {
            mem.writeBytes(ptr + HEADER_SIZE + align(keyLen), valBytes);
        }
        ...
        static boolean keyEquals(long ptr, byte[] keyBytes, GridUnsafeMemory mem) {
            ...
            return len == keyBytes.length && mem.compare(ptr + HEADER_SIZE, keyBytes);
        }
    }
}