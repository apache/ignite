...
public abstract class GridCacheAdapter<K, V> implements IgniteInternalCache<K, V>, Externalizable {
    ...
    @Override public void clear(K key) throws IgniteCheckedException {
        clear(Collections.singleton(key), false);
    }

    /** {@inheritDoc} */
    @Override public void clearAll(Set<? extends K> keys) throws IgniteCheckedException {
        clearAsync(keys, false).get();
    }

    /** {@inheritDoc} */
    @Override public IgniteInternalFuture<?> clearAsync() {
        return clearAsync(null, false);
    }

    /** {@inheritDoc} */
    @Override public IgniteInternalFuture<?> clearAsync(K key) {
        return clearAsync(Collections.singleton(key), false);
    }

    /** {@inheritDoc} */
    @Override public IgniteInternalFuture<?> clearAllAsync(Set<? extends K> keys) {
        return clearAsync(keys, false);
    }

    /**
     * @param keys Keys to clear.
     * @param nearOnly Clear near cache only.
     * @throws IgniteCheckedException In case of error.
     */
    private void clear(@Nullable Set<? extends K> keys, boolean nearOnly) throws IgniteCheckedException {
        if (!nearOnly)
     ...
     * @return Future.
     */
    private IgniteInternalFuture<?> clearAsync(@Nullable final Set<? extends K> keys, boolean nearOnly) {
        IgniteInternalFuture<?> fut = null;

        if (!nearOnly)
            fut = executeClearTask(keys, false);

        if (fut != null) {
            return fut.chain(new CX1<IgniteInternalFuture<?>, Object>() {
            ...
            return executeClearTask(keys, true);
    }
    ...
    private static class ClearTask<K> extends ComputeTaskAdapter<Object, Object> {
        ...
        /** */
        public static final IgniteProductVersion NEAR_JOB_SINCE = IgniteProductVersion.fromString("1.4.0");
