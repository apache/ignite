...
public abstract class GridCacheAdapter<K, V> implements IgniteInternalCache<K, V>, Externalizable {
    ...
    /** {@inheritDoc} */
    @Override public void setTtl(K key) {
    ...
    /** {@inheritDoc} */
    @Override public void setTtl(K key, ExpiryPolicy expiryPolicy) {

    }

    protected void setTtl0(final K key, long ttl) {
        GridCacheEntryEx entry = entryEx(key);
        try {
            // if the data isn't loaded in, we don't get expiry date
            entry.unswap(false);
...
//            return null;
    ...
    /** {@inheritDoc} */
    @Override public IgniteInternalFuture<Void> setTtlAsync(K key, long ttl) {
        final boolean statsEnabled = ctx.statisticsEnabled();
        ...
        IgniteInternalFuture<Void> fut = setTtlAsync0(key, ttl);

        if (statsEnabled)
            fut.listen(new UpdateRemoveTimeStatClosure<Void>(metrics0(), start));

        return fut;
    }

    protected IgniteInternalFuture<Void> setTtlAsync0(K key, long ttl) {
