...
public abstract class GridCacheAdapter<K, V> implements IgniteInternalCache<K, V>, Externalizable {
        ...
        try {
            ...
            if (!ctx.isLocal()) {
                ...
                if (modes.offheap || modes.swap) {
                    ...
                    cacheVal = offheapMgr.readValue(cacheKey, part);
                }
            }
            else
    ...
    @Nullable private CacheObject localCachePeek0(KeyCacheObject key,
        ...
        if (offheap || swap) {
            ...
            return offheapMgr.readValue(key, ctx.affinity().partition(key));
        }
        ...
        return asyncOp(new AsyncOp<V>() {
            @Override public IgniteInternalFuture<V> op(IgniteTxLocalAdapter tx) {
                try {
                    ...
                    return new GridFinishedFuture<V>(e);
                }
                ...
                if (previousState.active() && !state.active()) {
                    while (true) {
                        try {
                            ...
                            ctx.shared().exchange().dumpPendingObjects();
                        }
                    }
    ...
    protected void checkState() throws IgniteCheckedException {
        if (!state().active())
            throw new CacheInvalidStateException("Cache is inactive");
    }
