...
public abstract class GridCacheAdapter<K, V> implements IgniteInternalCache<K, V>, Externalizable {
    ...
    @Override public final IgniteInternalFuture<Boolean> containsKeyAsync(K key) {
        ...
        boolean readRepair = !ctx.systemTx() && !ctx.isLocal() && !ctx.readThrough() && ctx.config().getBackups() > 0 && !ctx.isNear() && !ctx.mvccEnabled() /* TODO replace with "opCtx != null && opCtx.readRepair()" */;
    ...
    @Override public final IgniteInternalFuture<Boolean> containsKeysAsync(final Collection<? extends K> keys) {
        ...
        return repairableGetAllAsync(
            ...
            !ctx.systemTx() && !ctx.isLocal() && !ctx.readThrough() && ctx.config().getBackups() > 0 && !ctx.isNear() && !ctx.mvccEnabled() /* TODO replace with "opCtx != null && opCtx.readRepair()" */,
            /*skip values*/true,
