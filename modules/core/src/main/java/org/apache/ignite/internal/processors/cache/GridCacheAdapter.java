...
public abstract class GridCacheAdapter<K, V> implements GridCache<K, V>,
     ...
     */
    public boolean putx(final K key, final V val, final CacheEntryPredicate[] filter)
        throws IgniteCheckedException {
        boolean statsEnabled = ctx.config().isStatisticsEnabled();
     ...
     * @return Put future.
     */
    public IgniteInternalFuture<?> putAllAsync(final Map<? extends K, ? extends V> m,
        @Nullable final CacheEntryPredicate... filter) {
        if (F.isEmpty(m))
            return new GridFinishedFuture<Object>();
     ...
     * @throws IgniteCheckedException If failed.
     */
     public boolean removex(final K key, @Nullable final CacheEntryPredicate... filter) throws IgniteCheckedException {
        boolean statsEnabled = ctx.config().isStatisticsEnabled();
     ...
     * @return Putx operation future.
     */
    public IgniteInternalFuture<Boolean> removexAsync(final K key, @Nullable final CacheEntryPredicate... filter) {
        final boolean statsEnabled = ctx.config().isStatisticsEnabled();
