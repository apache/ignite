...
public class PureJavaCrc32 {
     ...
     * @param len Length.
     */
    public static int calcCrc32(ByteBuffer b, final int len) {
        return calcCrc32(0xffffffff, b, len);
    }
     ...
     * @param len Length.
     */
    private static int calcCrc32(int crc0, ByteBuffer b, final int len) {
        int crc = crc0;
        ...
        for (; i < len - remainder; i += 8) {
            final int x = crc
                    ^ ((((b.get() << 24) >>> 24) + ((b.get() << 24) >>> 16))
                    + (((b.get() << 24) >>> 8) + (b.get() << 24)));

            crc = ((T[((x << 24) >>> 24) + 0x700] ^ T[((x << 16) >>> 24) + 0x600])
                    ^ (T[((x << 8) >>> 24) + 0x500] ^ T[(x >>> 24) + 0x400]))
                    ^ ((T[((b.get() << 24) >>> 24) + 0x300] ^ T[((b.get() << 24) >>> 24) + 0x200])
                    ^ (T[((b.get() << 24) >>> 24) + 0x100] ^ T[((b.get() << 24) >>> 24)]));
        }

        /* loop unroll - duff's device style */
        switch (remainder) {
            case 7:
                crc = (crc >>> 8) ^ T[((crc ^ b.get()) << 24) >>> 24];
            case 6:
                crc = (crc >>> 8) ^ T[((crc ^ b.get()) << 24) >>> 24];
            case 5:
                crc = (crc >>> 8) ^ T[((crc ^ b.get()) << 24) >>> 24];
