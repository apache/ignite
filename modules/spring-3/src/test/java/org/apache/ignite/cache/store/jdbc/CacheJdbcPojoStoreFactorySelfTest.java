...
public class CacheJdbcPojoStoreFactorySelfTest extends GridCommonAbstractTest {
    ...
    public void testCacheConfiguration() throws Exception {
        try (Ignite ignite = Ignition.start("modules/spring-3/src/test/config/node.xml")) {
            try (Ignite ignite1 = Ignition.start("modules/spring-3/src/test/config/node1.xml")) {
    ...
    public void testIncorrectBeanConfiguration() throws Exception {
        GridTestUtils.assertThrows(log, new Callable<Object>() {
            @Override public Object call() throws Exception {
                try(Ignite ignite = Ignition.start("modules/spring-3/src/test/config/pojo-incorrect-store-cache.xml")) {
