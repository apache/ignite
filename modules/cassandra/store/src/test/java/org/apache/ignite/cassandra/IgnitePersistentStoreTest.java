...
import org.apache.ignite.cassandra.utils.CacheStoreHelper;
...
public class IgnitePersistentStoreTest {
    ...
    public void loadCacheTest() {
        ...
        try (Ignite ignite = Ignition.start("org/apache/ignite/tests/persistence/pojo/ignite-config.xml")) {
            ...
            loadCacheByQuery(personCache3, qry);

            personCache3.clear();

            Statement stmt = new SimpleStatement(qry);
            loadCacheByStatement(personCache3, stmt);

            personCache3.clear();

            loacCacheByDefaultQuery(personCache3);
    ...
    private void loadCacheByQuery(IgniteCache<PersonId, Person> personCache3, String qry) {
        personCache3.loadCache(null, qry);
        int size = personCache3.size(CachePeekMode.ALL);
        Assert.assertEquals("Cache data was incorrectly loaded from Cassandra table by '"+qry+"'", 3, size);
    }

    private void loadCacheByStatement(IgniteCache<PersonId, Person> personCache3, Statement stmt) {
    ...
    private void loacCacheByDefaultQuery(IgniteCache<PersonId, Person> personCache3) {
        personCache3.loadCache(null);
        int size = personCache3.size(CachePeekMode.ALL);
        Assert.assertEquals("Cache data was incorrectly loaded from Cassandra. ", TestsHelper.getBulkOperationSize(), size);
    }
