...
class VisorConfigurationCommand {
    ...
    def config(args: String) {
        breakable {
            ...
            val execSvc = cfg.executeService()

            execSvcT += ("Executor service", safe(execSvc.executeService(), DFLT))
            execSvcT += ("Executor service shutdown", bool2Str(execSvc.executeServiceShutdown()))
            execSvcT += ("System executor service", safe(execSvc.systemExecuteService(), DFLT))
            execSvcT += ("System executor service shutdown", bool2Str(execSvc.systemExecuteServiceShutdown()))
            execSvcT += ("Peer-to-Peer executor service", safe(execSvc.p2pExecuteService(), DFLT))
            execSvcT += ("Peer-to-Peer executor service shutdown", bool2Str(execSvc.p2pExecuteServiceShutdown()))
