...
import org.gridgain.grid.kernal.visor.tasks.VisorEventsCollectTask.VisorEventsCollectArgs
    ...
    private def list(p: Long, taskName: String, reverse: Boolean, all: Boolean) {
        breakable {
            try {
                ...
                val evts = grid.compute(prj).execute(classOf[VisorEventsCollectTask],
    ...
    private def task(taskName: String, reverse: Boolean, all: Boolean) {
        breakable {
            ...
            try {
                ...
                val evts = grid.compute(prj).execute(classOf[VisorEventsCollectTask], toTaskArgument(prj.nodes.map(_.id()),
