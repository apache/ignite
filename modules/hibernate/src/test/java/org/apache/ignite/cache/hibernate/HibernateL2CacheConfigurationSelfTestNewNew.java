...
public class HibernateL2CacheConfigurationSelfTest extends GridCommonAbstractTest {
    ...
    public void testAlina(){
        SessionFactory sesFactory = startHibernate(getTestIgniteInstanceName(0));

        try {
            Session ses = sesFactory.openSession();

            //Creating Entity1 and Entity2
            try {
                Transaction tx = ses.beginTransaction();

                ses.save(new Entity1(0,"Entity1#Name1"));
                ses.save(new Entity2(1,"Entity2#Name1"));
            ...
            printEntities(sesFactory);assertEquals("Entity1#Name1",extractEntityNameFromRegion(sesFactory,"cache1",1));

            assertEquals(1,grid(0).cache("cache1").size());
            assertEquals(1,grid(0).cache("cache2").size());
            assertEquals("Entity1#Name1",extractEntityNameFromRegion(sesFactory,"cache1",1));
            assertEquals("Entity2#Name1",extractEntityNameFromRegion(sesFactory,"cache2",1));
            ...
            try {
                ...
                e1.setName("Entity1#Name1-UPDATED");
                ses.update(e1);

                ses.save(new Entity2(2,"Entity2#Name2-ADDED"));
            ...
            printEntities(sesFactory);

            assertEquals(1,grid(0).cache("cache1").size());
            assertEquals(2,grid(0).cache("cache2").size());
            assertEquals("Entity1#Name1-UPDATED",extractEntityNameFromRegion(sesFactory,"cache1",1));
            assertEquals("Entity2#Name1",extractEntityNameFromRegion(sesFactory,"cache2",1));
            assertEquals("Entity2#Name2-ADDED",extractEntityNameFromRegion(sesFactory,"cache2",2));
            ...
            try {
                ...
                e2.setName("Entity2#Name2-UPDATED-2");
                ses.update(e2);

                ses.save(new Entity1(3,"Entity1#Name2-ADDED"));

                Entity1 e1 = (Entity1) ses.load(Entity1.class, 1);
                e1.setName("Entity1#Name1-UPDATED-2");
                ses.update(e1);
            ...
            assertEquals(2,grid(0).cache("cache2").size());
            assertEquals("Entity1#Name1-UPDATED-2",extractEntityNameFromRegion(sesFactory,"cache1",1));
            assertEquals("Entity1#Name2-ADDED",extractEntityNameFromRegion(sesFactory,"cache1",2));
            assertEquals("Entity2#Name2-UPDATED-2",extractEntityNameFromRegion(sesFactory,"cache2",1));
            assertEquals("Entity2#Name2-ADDED",extractEntityNameFromRegion(sesFactory,"cache2",2));
    ...
    private  String extractEntityNameFromRegion(SessionFactory sesFactory, String regionName, Integer id) {
        String entityName = null;
        Session  ses = sesFactory.openSession();
        try{
            Iterator<Cache.Entry<Object,Object>> iterator = grid(0).cache(regionName).iterator();
    ...
    private static void printEntities(SessionFactory sesFactory) {
        Session    ses = sesFactory.openSession();
        System.out.println("========================================");
        try{
        ...
        for (Entity1 e1 : list1) {
            ...
            System.out.println(" Entity1 id is " + e1.getId() + " name: " + e1.getName());
        }
        ...
        for (Entity2 e2 : list2) {
            ...
            System.out.println(" Entity2 id is " + e2.getId() + " name: " + e2.getName());
        }
    ...
    private static void printEntities(SessionFactory  sessionFactory, String regionName){
        SecondLevelCacheStatistics stats = sessionFactory.getStatistics()
                .getSecondLevelCacheStatistics(regionName);
        if (stats!=null) {
            Map cacheEntries = stats.getEntries();
            System.out.println("/n/n/nEntities in cache in region '" + regionName + "':");
            for (Object key : cacheEntries.keySet()) {
                System.out.println("/n[" + key + "," + cacheEntries.get(key) + "]");
            }
        }
    }
