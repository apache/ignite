...
import org.apache.ignite.gatling.action.ActionBaseNew
import org.apache.ignite.gatling.api.IgniteApi

case class StartClientAction(requestName: Expression[String],
                             next: Action,
                             ctx: ScenarioContext
                            ) extends ActionBaseNew with NameGen {
  ...
  override protected def execute(session: Session): Unit = withSession(session) {
    for {
      ...
      val func = IgniteApi(components.igniteProtocol) _

      call(func, resolvedRequestName, session, (session, igniteApi: Option[IgniteApi]) => {
        igniteApi match {
          case Some(igniteApi) => session.set(IGNITE_API_SESSION_KEY, igniteApi)
        }
      })
    }
  }
