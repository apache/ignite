...
case class TransactionCommitAction(requestName: Expression[String],
  ...
  override protected def execute(session: Session): Unit = {
    logger.debug(s"session user id: #${session.userId}, commit")
    ...
    (for {
      ...
      .commit()(
        _ => logAndExecuteNext(session.remove("transactionApi"), resolvedRequestName, startTime,
          ctx.coreComponents.clock.nowMillis, OK, next, None, None),
        ex => {
          logger.error(s"session user id: #${session.userId}, commit failed", ex)
