 ...
 */
case class CacheNodeApi[K, V](wrapped: IgniteCache[K, V])(implicit val ec: ExecutionContext) extends CacheApi[K, V] with StrictLogging {
  ...
  override def invokeAll[T](
    ...
    Try {
      wrapped.invokeAll[T](map.asJava, arguments)
    }
      .map(_.asScala.toMap)
      .fold(f, s)
  }
