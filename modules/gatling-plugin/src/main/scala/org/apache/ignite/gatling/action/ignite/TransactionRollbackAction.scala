...
case class TransactionRollbackAction(requestName: Expression[String],
                                     next: Action,
                                     ctx: ScenarioContext
                            ) extends ChainableAction with NameGen with ActionBase with StrictLogging {

  override val name: String = genName("rollback")

  override protected def execute(session: Session): Unit = {
    logger.debug(s"session user id: #${session.userId}, rollback")

    val transactionApi: TransactionApi = session("transactionApi").as[TransactionApi]

    (for {
      resolvedRequestName <- requestName(session)
      startTime <- ctx.coreComponents.clock.nowMillis.success
    } yield transactionApi
      .rollback()(
        _ => {
          logger.debug(s"session user id: #${session.userId}, after rollback")
          logAndExecuteNext(session.remove("transactionApi"), resolvedRequestName, startTime,
            ctx.coreComponents.clock.nowMillis, OK, next, None, None)
        },
        ex => {
          logger.error(s"session user id: #${session.userId}, after rollback", ex)
