...
from ducktests_remote.transport import ProxiedTransport, make_tarball
...
def execute(ctx):  # pylint: disable=too-many-locals
    ...
    plans = []
    for name in dists:
        manifest = build_manifest(dist_dir / name, checksum=use_checksum)
        plans.append((name, manifest))

    _print_cost(ctx, plans, nodes)

    overall = []
    for name, manifest in plans:
...
def build_manifest(path, *, checksum=False):
    """
    ...
    for entry in sorted(root.rglob("*")):
        ...
        total += stat.st_size
        if checksum:
            entries.append("%s\0%d\0%s" % (rel, stat.st_size, _sha256(entry)))
        else:
            entries.append("%s\0%d\0%d" % (rel, stat.st_size, int(stat.st_mtime)))
    digest = hashlib.sha256("\n".join(entries).encode("utf-8")).hexdigest()
    return {"hash": digest, "files": len(entries), "bytes": total,
...
def _deploy_one(ctx, dist_dir, name, manifest, install_root, nodes):
    args = ctx.args
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        make_tarball(Path(dist_dir) / name, archive)
