...
class PdsCompatibilityTest(IgniteTest):
    ...
    """
    APP_CLASS = "org.apache.ignite.internal.ducktest.tests.pds_compatibility_test.DictionaryCacheApplication"
    CACHE_NAME = "dictionary"
    ...
    def test_pds_compatibility(self, ignite_version_1, ignite_version_2):
        ...
        ignite1 = IgniteService(self.test_context, server_configuration_1, num_nodes=num_nodes)
        ignite2 = IgniteService(self.test_context, server_configuration_2, num_nodes=num_nodes)
        ignite2.nodes = ignite1.nodes.copy()

        ignite1.start()

        self.run_application(ignite1, self.LOAD_OPERATION)

        ignite1.stop()
        ignite1.free()

        ignite2.start(clean=False)

        self.run_application(ignite2, self.CHECK_OPERATION)

    def run_application(self, ignite, operation):
        ...
        IgniteApplicationService(self.test_context, config=app_config,
                                 ...
                                 params={"cacheName": self.CACHE_NAME, "operation": operation}).run()
