...
class RebalancePersistentTest(IgniteTest):
    ...
    def test_node_join(self, ignite_version, backups, cache_count, entry_count, entry_size, preloaders,
        ...
        self.logger.warn(control_utility.baseline())
    ...
    #     control_utility.deactivate()
    #
    #     self.logger.warn(f'DB size after rebalance: {get_database_size_mb(ignites.nodes, ignites.database_dir)}')
    #
    #     return get_result([node], preload_time, 1, entry_count, entry_size)


def get_database_size_mb(nodes: dict, database_dir: str, check_error=False):
    """
    ...
    for node in nodes:
        ...
        cmd = "du -ms %s | awk {'print $1'}" % os.path.join(database_dir, cons_id)
