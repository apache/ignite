...
package org.apache.ignite.compatibility.legacy.persistence;
...
public class PersistenceBasicCompatibilityTest extends IgnitePersistenceCompatibilityAbstractTest {
    ...
    protected void doTestStartupWithOldVersion(String igniteVer, boolean compactFooter) throws Exception {
        ...
        try {
            ...
            IgniteEx ex = startGrid(1, igniteVer, new ConfigurationClosure(compactFooter), new PostStartupClosure());

            stopAllGrids();

            IgniteEx ignite = startGrid(0);

            TestCase.assertEquals(1, ignite.context().discovery().topologyVersion());
    ...
    public static void validateResultingCacheData(Cache<Object, Object> cache) {
        ...
        for (int i = 0; i < 10; i++)
            TestCase.assertEquals("data" + i, cache.get(i));

        TestCase.assertEquals("2", cache.get("1"));
        TestCase.assertEquals(2, cache.get(12));
        TestCase.assertEquals(2L, cache.get(13L));
        TestCase.assertEquals("Enum_As_Key", cache.get(TestEnum.A));
        TestCase.assertEquals(TestEnum.B, cache.get("Enum_As_Value"));
        TestCase.assertEquals(TestEnum.C, cache.get(TestEnum.C));
        TestCase.assertEquals(new TestSerializable(42), cache.get("Serializable"));
        TestCase.assertEquals("Serializable_As_Key", cache.get(new TestSerializable(42)));
        TestCase.assertEquals(new TestExternalizable(42), cache.get("Externalizable"));
        TestCase.assertEquals("Externalizable_As_Key", cache.get(new TestExternalizable(42)));
        TestCase.assertEquals(new TestStringContainerToBePrinted("testStringContainer"), cache.get("testStringContainer"));
        TestCase.assertEquals(UUID.fromString("B851B870-3BA7-4E5F-BDB8-458B42300000"),
