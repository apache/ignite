...
class BinaryType {
        ...
        for (let field of binaryType.fields) {
            ...
            if (this.hasField(fieldId)) {
                if (this.getField(fieldId).typeCode !== field.typeCode) {
                    throw internal_1.Errors.IgniteClientError.serializationError(true, Util.format('type conflict for field "%s" of complex object type "%s"', field.name, this._name));
                }
            }
            else {
                this.setField(field);
    ...
    static _calculateId(name) {
        return internal_1.BinaryUtils.hashCodeLowerCase(name);
    }
    _write(buffer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            internal_1.BinaryCommunicator.writeString(buffer, null);
            // fields count
    ...
    _read(buffer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // type name
            this._name = internal_1.BinaryCommunicator.readString(buffer);
            // affinity key field name
            internal_1.BinaryCommunicator.readString(buffer);
            // fields count
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._isEnum) {
                ...
                for (let i = 0; i < valuesCount; i++) {
                    this._enumValues[i] = [internal_1.BinaryCommunicator.readString(buffer), buffer.readInteger()];
                }
            }
        });
    }
...
class BinaryTypeBuilder {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (type) {
                ...
                if (schemaId !== null) {
                    ...
                    if (!result._schema) {
                        throw internal_1.Errors.IgniteClientError.serializationError(false, Util.format('schema id "%d" specified for complex object of type "%s" not found', schemaId, type.name));
                    }
    ...
    static fromObject(jsObject, complexObjectType = null) {
        if (complexObjectType) {
            ...
            result._fromComplexObjectType(new internal_1.ComplexObjectType(jsObject), jsObject);
            return result;
        }
    }
    static fromComplexObjectType(complexObjectType, jsObject) {
        let result = new BinaryTypeBuilder();
        const typeInfo = internal_1.BinaryTypeStorage.getByComplexObjectType(complexObjectType);
        if (typeInfo) {
            ...
            internal_1.BinaryTypeStorage.setByComplexObjectType(complexObjectType, result._type, result._schema);
        }
        return result;
    }
    getTypeId() {
    ...
    _setFields(complexObjectType, objectTemplate, jsObject) {
        let fieldType;
        for (let fieldName of internal_1.BinaryUtils.getJsObjectFieldNames(objectTemplate)) {
            fieldType = complexObjectType._getFieldType(fieldName);
            if (!fieldType && jsObject[fieldName]) {
                fieldType = internal_1.BinaryUtils.calcObjectType(jsObject[fieldName]);
            }
            this.setField(fieldName, internal_1.BinaryUtils.getTypeCode(fieldType));
        }
    }
}
