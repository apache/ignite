...
#ifndef _IGNITE_IMPL_THIN_PARSER
#define _IGNITE_IMPL_THIN_PARSER
    ...
    namespace impl
    {
        namespace thin
        {
            ...
            public:
                 ...
                 */
                Parser(const ProtocolVersion& ver, int32_t cap = DEFAULT_MEM_ALLOCATION) :
                    protocolVer(ver),
                    inMem(cap),
                    outMem(cap),
                    outStream(&outMem)
                ...
                void Decode(MsgT& msg, const std::vector<int8_t>& buf)
                {
                    ...
                    interop::InteropInputStream inStream(&inMem);
            ...
            private:
                ...
                interop::InteropUnpooledMemory inMem;

                /** Output operational memory. */
                interop::InteropUnpooledMemory outMem;

                /** Output stream. */
                interop::InteropOutputStream outStream;
            };
        }
    }
}

#endif //_IGNITE_IMPL_THIN_PARSER