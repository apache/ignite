...
When reading Complex Object from cache, `pyignite` will either create the
`Person` class automatically, or reuse the user-defined `Person` class,
depending on the program flow. The code and resulting values should be
the same.
...
If you intend to use custom class for reading existing Complex Objects
rather than writing them first, you must register said class explicitly
with your client:
...
And of course you can use an autogenerated data class after reading a Complex
Object:
...
One obvious example of Complex Object usage is SQL tables. Normally SQL data
is accessed via queries (see `SQL`_), so we will consider the following
