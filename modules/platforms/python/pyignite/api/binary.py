...
def get_binary_type(
    client: Client, binary_type: Union[str, int], query_id=None,
) -> APIResult:
    """
    ...
    response_head_type, recv_buffer = response_head_struct.parse(client)
    response_head = response_head_type.from_buffer_copy(recv_buffer)
    response_parts = []
    if response_head.type_exists:
        resp_body_type, resp_body_buffer = body_struct.parse(client)
        ...
        if resp_body.is_enum:
            resp_enum, resp_enum_buffer = enum_struct.parse(client)
            response_parts.append(('enums', resp_enum))
            recv_buffer += resp_enum_buffer
        resp_schema_type, resp_schema_buffer = schema_struct.parse(client)
...
def put_binary_type(
    client: Client, type_name: str, affinity_key_field: str=None,
