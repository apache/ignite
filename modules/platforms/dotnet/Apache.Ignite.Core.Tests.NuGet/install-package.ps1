...
(Get-Content packages.config) -replace 'id="Apache.Ignite(.*?)" version=".*?"', ('id="Apache.Ignite$1" version="' + $ver + '"') | Out-File packages.config

(Get-Content Apache.Ignite.Core.Tests.NuGet.csproj) -replace '<HintPath>packages\\Apache.Ignite(.*?)\.[\d.]+\\', ('<HintPath>packages\Apache.Ignite$1.' + "$ver\") | Out-File Apache.Ignite.Core.Tests.NuGet.csproj