...
namespace Apache.Ignite.Core.Impl.Compute.Closure
{
    ...
    internal abstract class ComputeAbstractClosureTask<TA, T, TR> : IComputeTask<TA, T, TR>
    {
        ...
        public ComputeJobResultPolicy OnResult(IComputeJobResult<T> res, IList<IComputeJobResult<T>> rcvd)
        {
            ...
            if (err != null)
            {
                if (IsFailoverException(err) || IsFailoverException(err.InnerException))
        ...
        /// </summary>
        private static bool IsFailoverException(Exception err)
        {
            return err is ComputeExecutionRejectedException || err is ClusterTopologyException || 
