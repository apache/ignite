...
import org.apache.ignite.internal.visor.encryption.VisorStartReencryptionTask;
...
/**
 * Start cache group reencryption subcommand.
 */
public class ReencryptionStartCommand implements Command<String> {
    /** Cache group name. */
    ...
    @Override public Object execute(GridClientConfiguration clientCfg, Logger log) throws Exception {
        try (GridClient client = Command.startClient(clientCfg)) {
            Map<UUID, Object> resErrs = executeTaskByNameOnNode(
                client,
                VisorStartReencryptionTask.class.getName(),
                CU.cacheId(grpName),
                BROADCAST_UUID,
                clientCfg
            );
            ...
            for (Map.Entry<UUID, Object> entry : resErrs.entrySet()) {
                ...
                if (entry.getValue() instanceof Throwable) {
                    msg = "failed to start re-encryption of the cache group \"" + grpName +
                        "\" (" + ((Throwable)entry.getValue()).getMessage() + ").";
                }
                else {
                    msg = "re-encryption of the cache group \"" + grpName + "\" has " +
                        (((boolean)entry.getValue()) ? "" : "already ") + "been started.";
                }
    ...
    @Override public void printUsage(Logger log) {
        Command.usage(log, "Start cache group re-encryption:", ENCRYPTION,
            REENCRYPTION_START.toString(), "cacheGroupName");
    }

    /** {@inheritDoc} */
    @Override public String name() {
        return REENCRYPTION_START.name();
    }
}
