 ...
 * via the methods of {IgniteClient} objects.
...
class CacheClient {
     ...
     *
     * @param {ObjectType | integer} type - (optional) type of the keys in the cache:
     ...
     *   Eg. {UnsupportedTypeError} - if the provided type is not supported.
     */
    setKeyType(type) {
     ...
     * according to the mapping table defined in the description of the {ObjectType} class.
     *
     * @param {ObjectType | integer} type - (optional) type of the values in the cache:
     *   - either an instance of object type
     *   - or a type code (means object type with this type code and with default subtype, if applicable)
     *   - or null (means the type is not specified).
     *
     ...
     * TBD
     *
     * @param {boolean} on - (optional) If true or not specified: switch on "binary" mode.
     *   If false: switch off "binary" mode.
     *
     * @return {CacheClient} - the same instance of the cache client.
     *
     * @throws any of the {IgniteClientError} subclasses. TBD
     */
     ...
     *
     * @throws any of the {IgniteClientError} subclasses.
     *   Eg. {IllegalArgumentError} - if the key is null.
     *       {TypeCastError} - if the key type is different from the type specified for the keys of the cache client.
     *       {UnsupportedTypeError} - if the key type is not supported.
     */
    async get(key) {
     ...
     *
     * @throws any of the {IgniteClientError} subclasses.
     *   Eg. {IllegalArgumentError} - if the key/value is null.
     *       {TypeCastError} - if the key/value type is different from the type specified
     *                         for the keys/values of the cache client.
     *       {UnsupportedTypeError} - if the key/value type is not supported.
     */
    async put(key, value) {
        ArgumentChecker.notNull(key, "key");
