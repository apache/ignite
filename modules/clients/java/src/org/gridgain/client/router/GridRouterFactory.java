 ...
 * You can use {@link GridTcpRouterConfiguration} and {@link GridHttpRouterConfiguration}
 * to set configuration parameters and pass them to
 * {@link #startTcpRouter(GridTcpRouterConfiguration)} and {@link #startHttpRouter(GridHttpRouterConfiguration)}.
 * <p>
 * See {@link GridTcpRouter} and {@link GridHttpRouter} for example on how to configure and start embedded router.
...
public final class GridRouterFactory {
    ...
    /**
     * Starts a HTTP router with given configuration.
     * <p>
     * Starting router will be assigned a randomly generated UUID which can be obtained
     * by {@link GridTcpRouter#id()} method. Later this instance could be obtained via
     * {@link #tcpRouter} method.
     ...
     * @throws GridException If router start failed.
     */
    public static GridHttpRouter startHttpRouter(GridHttpRouterConfiguration cfg) throws GridException {
        GridHttpRouterImpl router = new GridHttpRouterImpl(cfg);

        router.start();

        GridHttpRouterImpl old = httpRouters.putIfAbsent(router.id(), router);

        assert old == null : "UUIDs collision happens [httpRouters=" + httpRouters + ", router=" + router + ']';

        return router;
    }

    /**
     * Stops particular HTTP router.
     *
     * @param httpRouterId Router identifier to close.
     */
    public static void stopHttpRouter(UUID httpRouterId) {
        GridHttpRouterImpl router = httpRouters.remove(httpRouterId);
    ...
    /**
     * Returns HTTP router with given id.
     *
     * @param id Router Id.
     * @return Router with the given id or {@code null} if router not found.
     */
    @Nullable public static GridHttpRouter httpRouter(UUID id) {
        return httpRouters.get(id);
    }

    /**
     * Returns collection of all currently running HTTP routers.
     *
     * @return Collection of currently running {@link GridHttpRouter}s.
     */
    public static Collection<GridHttpRouter> allHttpRouters() {
        return new ArrayList<GridHttpRouter>(httpRouters.values());
    }
    ...
    public static void stopAllRouters() {
        ...
        for (Iterator<GridHttpRouterImpl> it = httpRouters.values().iterator(); it.hasNext(); ) {
            GridHttpRouterImpl router = it.next();
