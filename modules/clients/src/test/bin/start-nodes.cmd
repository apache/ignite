...
echo call ant -DGG_HOME="%GG_HOME%" -DTMP_DIR="%TEMP%" -DJAVA_HOME="%JAVA_HOME%" -v -f %SCRIPT_DIR%\..\build\build.xml %ANT_TARGET%

call ant -DGG_HOME="%GG_HOME%" -DTMP_DIR="%TEMP%" -DJAVA_HOME="%JAVA_HOME%" -v -f %SCRIPT_DIR%\..\build\build.xml %ANT_TARGET%
...
FOR /L %%G IN (1,1,%NODES_COUNT%) DO start "Cache Node #%%G" /low /MIN cmd /C "%GG_HOME%\os\bin\ggstart.bat -v %GG_HOME%\modules\os\modules\clients\src\test\resources\spring-cache.xml >> %GG_HOME%\work\log\cache-node-%%G.log 2>&1"
