...
public:
    ...
    void run(org::gridgain::grid::kernal::processors::rest::client::message::ObjectWrapperType messageType) {
        iters = 0;

        try {
            bool unmarshal = vm["unmarshal"].as<bool>();

            org::gridgain::grid::kernal::processors::rest::client::message::ObjectWrapper marshalledObject;
            ...
            if ((messageType > NONE && messageType <= STRING) || messageType == UUID) {
                ...
                while (++iters != maxiterations) {
                    GridClientObjectWrapperConvertor::wrapSimpleType(value,
                            marshalledObject);
                ...
                case CACHE_REQUEST: {
                    ...
                    while (++iters != maxiterations) {
                        GridClientProtobufMarshaller::wrap(cmd,
                                marshalledObject);
                        if (unmarshal) {
...
int main(int argc, const char** argv) {
    ...
    desc.add_options()("help", "produce help message")("threads", value<int>(),
            "Number of threads")("messagetype", value<int>(),
            "Type of messages to process, value from enum ObjectWrapperType in ClientMessages.pb.h")(
            "nummessages", value<int>(),
            "Number of messages to process per thread")("unmarshal",
            boost::program_options::value<bool>(),
            "Unmarshal marshalled messages (bool)");
    ...
    if ((org::gridgain::grid::kernal::processors::rest::client::message::ObjectWrapperType)
        vm["messagetype"].as<int>() != NONE) {
        std::vector<TestThreadPtr> workers;

        for (int i = 0; i < vm["threads"].as<int>(); i++) {
            workers.push_back(TestThreadPtr(
                new TestThread((org::gridgain::grid::kernal::processors::rest::client::message::ObjectWrapperType)
                    vm["messagetype"].as<int>())));
        }
        ...
        for (int itertype = (int)BOOL; itertype <= (int)TASK_BEAN; ++itertype) {
            ...
            for (int i = 0; i < vm["threads"].as<int>(); i++) {
                workers.push_back(TestThreadPtr(new TestThread(
                    (org::gridgain::grid::kernal::processors::rest::client::message::ObjectWrapperType)itertype)));
            }
