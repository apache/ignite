...
public class ContextAwareCallable<T> extends ContextAwareWrapper<Callable<T>> implements Callable<T> {
    ...
    /**
     * Creates a wrapper that stores a specified {@link Callable} along with the Context Snapshot bound
    ...
    /** The same as {@link #wrapIfActiveAttributesPresent(Callable)} but wraps each specified {@link Callable}. */
    public static <T> Collection<Callable<T>> wrapIfActiveAttributesPresent(Collection<? extends Callable<T>> tasks) {
        return tasks == null
            ? null
            : tasks.stream().map(ContextAwareCallable::wrapIfActiveAttributesPresent).collect(Collectors.toList());
    }
}
