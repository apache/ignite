 ...
 */
public class SimpleSecurityProcessor extends GridProcessorAdapter implements GridSecurityProcessor {
    /** Permissions. */
    private static final Map<SecurityCredentials, SecurityPermissionSet> PERMS = new ConcurrentHashMap<>();

    /** Node security data. */
    private final SimpleSecurityData nodeSecData;

    /** Users security data. */
    private final Collection<SimpleSecurityData> predefinedAuthData;
    ...
    @Override public void authorize(String name, SecurityPermission perm, SecurityContext securityCtx)
        ...
        if (!((SimpleSecurityContext)securityCtx).operationAllowed(name, perm))
    ...
    @Override public void start() throws IgniteCheckedException {
        ...
        for (SimpleSecurityData data : predefinedAuthData)
