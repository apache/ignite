 ...
 */
public class GridGgfsHadoopFileSystemHandshakeSelfTest extends GridCommonAbstractTest {
    /** IP finder. */
    ...
    private void startUp(boolean dfltGridName, boolean dfltGgfsName) throws Exception {
        Grid grid = G.start(gridConfiguration(dfltGridName, dfltGgfsName));

        GridGgfs ggfs = grid.ggfs(dfltGgfsName ? null : GGFS_NAME);
    ...
    private GridConfiguration gridConfiguration(boolean dfltGridName, boolean dfltGgfsName) throws Exception {
        ...
        ggfsCfg.setIpcEndpointConfiguration(GridTestUtils.jsonToMap("{type:'tcp', port:" + DFLT_IPC_PORT + "}"));
