...
public class CacheQueryEntityWithJsr310Java8DateTimeApiFieldsTest extends CacheQueryJsr310Java8DateTimeApiBaseTest {
     ...
     */
    public static class SqlFuncitons {
        /**
         * Converts the specified {@link LocalDateTime} object to a {@link Timestamp} object.
         *
         * @param localDateTime {@link LocalDateTime} object to convert.
    ...
    private static CacheConfiguration<Long, EntityWithJsr310Fields> createCacheConfig() {
        return CacheQueryEntityWithJsr310Java8DateTimeApiFieldsTest.<Long, EntityWithJsr310Fields>createCacheConfig(
            "entityWithJava8DataTimeFields", Long.class, EntityWithJsr310Fields.class
        ).setSqlFunctionClasses(SqlFuncitons.class);
    }
    ...
    /**
     * Tests selection of an entity.
     *
     * @throws Exception If failed.
     */
    public void testSelectEntity() throws Exception {
        SqlFieldsQuery qry = new SqlFieldsQuery("select _val from EntityWithJsr310Fields");
    ...
    /**
     * Tests that DATEDIFF SQL function fails for {@link LocalDateTime}
     * fields with the time part set to midnight.
     *
     * @throws Exception If failed.
     */
    public void testDateDiffForLocalDateTimeFieldAtMidnightThrows() throws Exception {
        GridTestUtils.assertThrows(
            log,
            () -> {
                SqlFieldsQuery qry = new SqlFieldsQuery(
                    "select DATEDIFF('DAY', locDateTime, CURRENT_DATE ()) from EntityWithJsr310Fields");
    ...
    /**
     * Tests that DATEDIFF SQL function works for a {@link LocalDateTime}
     * with the time part set to midnight field converted to
     * a {@link Timestamp} object via a custom SQL function.
     *
     * @throws Exception If failed.
     */
    public void testDateDiffForLocalDateTimeFieldAtMidnight() throws Exception {
        SqlFieldsQuery qry = new SqlFieldsQuery(
            "select DATEDIFF('DAY', localDateTimeToTimestamp(locDateTime), CURRENT_DATE ()) from EntityWithJsr310Fields");
    ...
    /**
     * Tests that selection of a {@link LocalTime} field.
     *
     * @throws Exception If failed.
     */
    public void testSelectLocalTimeField() throws Exception {
        SqlFieldsQuery qry = new SqlFieldsQuery("select locTime from EntityWithJsr310Fields");

        List<List<?>> qryResults = cache.query(qry).getAll();

        assertEquals(1, qryResults.size());
        assertEquals(LocalTime.class, qryResults.get(0).get(0).getClass());
    }

    /**
     * Tests that selection of a {@link LocalDate} field.
     *
     * @throws Exception If failed.
     */
    public void testSelectLocalDateField() throws Exception {
        SqlFieldsQuery qry = new SqlFieldsQuery("select locDate from EntityWithJsr310Fields");

        List<List<?>> qryResults = cache.query(qry).getAll();

        assertEquals(1, qryResults.size());
        assertEquals(LocalDate.class, qryResults.get(0).get(0).getClass());
    }

    /**
     * Tests that selection of a {@link LocalDateTime} field.
     *
     * @throws Exception If failed.
     */
    public void testSelectLocalDateTimeField() throws Exception {
        SqlFieldsQuery qry = new SqlFieldsQuery("select locDateTime from EntityWithJsr310Fields");

        List<List<?>> qryResults = cache.query(qry).getAll();

        assertEquals(1, qryResults.size());
        assertEquals(LocalDateTime.class, qryResults.get(0).get(0).getClass());
    }
    ...
    public void testSelectByAllJsr310Fields() {
        SqlFieldsQuery qry = new SqlFieldsQuery(
            "select _val from EntityWithJsr310Fields where locTime = ? and locDate = ? and locDateTime = ?"
