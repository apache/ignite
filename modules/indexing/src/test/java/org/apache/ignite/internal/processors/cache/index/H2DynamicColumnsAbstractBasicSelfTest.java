...
public abstract class H2DynamicColumnsAbstractBasicSelfTest extends DynamicColumnsAbstractTest {
    ...
    /** */
    public void testDropColumnPriorToCompoundIndex() throws Exception {
        try {
            run("CREATE TABLE test (pk_id INT PRIMARY KEY, a INT, b INT, id1 INT, id2 INT)");

            run("CREATE INDEX idx_on_test ON test(id1, id2)");

            run("INSERT INTO test (pk_id, a, b, id1, id2) VALUES (1, 1, 2, 3, 4)");
            ...
            String pkIdxName = "PUBLIC.idx_on_test".toUpperCase();
