...
//        execSql("DROP TABLE " + t(q(SCHEMA_NAME_3), TBL_NAME));
    ...
    private void verifyTbls() {
        IgniteH2Indexing ind = (IgniteH2Indexing)grid(0).context().query().getIndexing();

        Assert.assertNotNull(ind.dataTable(SCHEMA_NAME_1, TBL_NAME));
        Assert.assertNotNull(ind.dataTable(SCHEMA_NAME_2, TBL_NAME));
        Assert.assertNotNull(ind.dataTable(SCHEMA_NAME_4, TBL_NAME));
        // TODO: https://ggsystems.atlassian.net/browse/GG-25751
//        Assert.assertNotNull(ind.dataTable(SCHEMA_NAME_3, TBL_NAME));
    }
}
