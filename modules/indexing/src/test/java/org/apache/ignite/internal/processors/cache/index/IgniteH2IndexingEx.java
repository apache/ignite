...
public class IgniteH2IndexingEx extends IgniteH2Indexing {
    /**
     * Consumer for cache rows when creating a new index.
     * Mapping: Node name -> Index name -> Consumer.
     */
    private static final Map<String, Map<String, IgniteThrowableConsumer<CacheDataRow>>> idxCreateRowConsumer =
    ...
    static void clean(@Nullable String nodeNamePrefix) {
        if (nodeNamePrefix == null)
            idxCreateRowConsumer.clear();
        else
            idxCreateRowConsumer.entrySet().removeIf(e -> e.getKey().startsWith(nodeNamePrefix));
    }

    /**
     * Registering a consumer for cache rows when creating a new index.
     *
     ...
     */
    static void addIdxCreateRowConsumer(String nodeName, String idxName, IgniteThrowableConsumer<CacheDataRow> c) {
        idxCreateRowConsumer.computeIfAbsent(nodeName, s -> new ConcurrentHashMap<>()).put(idxName, c);
    }
}
