...
public class IgniteSqlCreateTableTemplateTest extends GridCommonAbstractTest {
     ...
     */
    @Test
    public void testCustomAffinityKeyMapperIsNotOverwritten() throws Exception {
        Ignite ignite = grid();
        IgniteCache cache = ignite.getOrCreateCache("test");

        createTable(cache, "CUSTOM", "CUSTOM_TEMPLATE");

        assertTrue(ignite.getOrCreateCache("CUSTOM_CACHE").getConfiguration(CacheConfiguration.class).getAffinityMapper() instanceof MockAffinityKeyMapper);
    }

    /**
     * Mock affinity mapper implementation.
     */
    private static class MockAffinityKeyMapper implements AffinityKeyMapper, Serializable {
