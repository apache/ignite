...
/**
 * Checks failed SQL query with the WITH clause (AKA Common Table Expressions (CTEs)).
 */
public class SqlFailedCteQueryTest extends AbstractIndexingCommonTest {
    /** */
    ...
    /**
     * Check that failed CTE query doesn't prevent the table drop and doesn't crash the
     * ignite node on table recreate if used via the Ignite API.
     *
     * @throws Exception If fails.
     */
    @Test
    public void testFailedCteQueryDoesNotCrashNode() throws Exception {
        node.createCache(new CacheConfiguration<>("T1")
            .setQueryEntities(List.of(new QueryEntity("java.lang.Long", "T1")))
            .setSqlSchema(SCHEMA));
        ...
        // Force statement parsing failure.
    ...
    /**
     * Check that failed CTE query doesn't prevent the table drop and doesn't prevent
     * the table recreate if used via the SQL API.
     */
    @Test
    public void testFailedCteQueryDoesNotPreventTableRecreate() {
        ...
        assertThrowsWithCause(() -> sql(
            "WITH cte AS (SELECT id FROM T2) " +
            "SELECT * FROM absent_table_to_emulate_failure"), IgniteSQLException.class);
