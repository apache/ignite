...
public abstract class GridIndexingSpiAbstractSelfTest extends GridCommonAbstractTest {
    ...
    public void testSpi() throws Exception {
        ...
        assertFalse(spi.query(typeBA.space(), "select * from B.A, A.B, A.A", Collections.emptySet(), typeBA, null).hasNext());

        try {
            spi.query(typeBA.space(), "select aa.*, ab.*, ba.* from A.A aa, A.B ab, B.A ba", Collections.emptySet(), typeBA, null).hasNext();
            fail("Enumerations of aliases in select block must be prohibited");
        } catch (IgniteCheckedException e) {
            // all fine
        }
        assertFalse(spi.query(typeAB.space(), "select ab.* from A.B ab", Collections.emptySet(), typeAB, null).hasNext());
        assertFalse(spi.query(typeBA.space(), "select   ba.*   from B.A  as ba", Collections.emptySet(), typeBA, null).hasNext());
        ...
        res = spi.query(typeAA.space(), "select aa.* from a aa order by aa.age", Collections.emptySet(), typeAA, null);
        ...
        res = spi.query(typeAB.space(), "select bb.* from b as bb order by bb.name", Collections.emptySet(), typeAB, null);
