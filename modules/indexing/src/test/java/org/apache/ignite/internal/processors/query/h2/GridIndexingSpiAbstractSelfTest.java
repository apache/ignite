...
public abstract class GridIndexingSpiAbstractSelfTest extends GridCommonAbstractTest {
    ...
    public void testSpi() throws Exception {
        ...
        assertFalse(hasLocalQueryResults(spi, typeAA.space(), "select * from A.A", typeAA));
        assertFalse(hasLocalQueryResults(spi, typeAB.space(), "select * from A.B", typeAB));
        assertFalse(hasLocalQueryResults(spi, typeBA.space(), "select * from B.A", typeBA));

        assertFalse(hasLocalQueryResults(spi, typeBA.space(), "select * from B.A, A.B, A.A", typeBA));
        ...
        assertFalse(hasLocalQueryResults(spi, typeAB.space(), "select ab.* from A.B ab", typeAB));

        assertFalse(hasLocalQueryResults(spi, typeBA.space(), "select   ba.*   from B.A  as ba", typeBA));
        ...
        try (GridCloseableIterator<IgniteBiTuple<Integer, Map<String, Object>>> res =
                 spi.queryLocalSql(typeBA.space(), "from a", null, Collections.emptySet(), typeBA, null)) {
    ...
    private static boolean hasLocalQueryResults(IgniteH2Indexing spi, String spaceName, String qry,
        TypeDesc type) throws IgniteCheckedException {

        try (GridCloseableIterator<IgniteBiTuple<Object, Object>> res =
                 spi.queryLocalSql(spaceName, qry, null, Collections.emptySet(), type, null)) {
            return res.hasNext();
        }
    }
