...
public class IgniteSqlSplitterSelfTest extends GridCommonAbstractTest {
    ...
    private void checkQueryWithGroupsAndAggrs(IgniteCache<Integer, Value> cache) {
        try (QueryCursor<List<?>> qry = cache.query(new SqlFieldsQuery(
            "SELECT count(snd), sum(snd), avg(snd), min(snd), max(snd) FROM Value GROUP BY fst ORDER BY fst"))) {
            List<List<?>> result = qry.getAll();

            assertEquals(6, result.size());

            List<?> row = result.get(0);
            assertEquals("count", 3L, ((Number)row.get(0)).longValue());
            assertEquals("sum", 9L, ((Number)row.get(1)).longValue());
            assertEquals("avg", 3.0d,  ((Number)row.get(2)).doubleValue(), 0.001);
            assertEquals("min", 3, ((Integer)row.get(3)).intValue());
            assertEquals("max", 3, ((Integer)row.get(4)).intValue());

            row = result.get(1);
            assertEquals("count", 3L, ((Number)row.get(0)).longValue());
            assertEquals("sum", 6L, ((Number)row.get(1)).longValue());
            assertEquals("avg", 2.0d,  ((Number)row.get(2)).doubleValue(), 0.001);
            assertEquals("min", 1, ((Integer)row.get(3)).longValue());
            assertEquals("max", 3, ((Integer)row.get(4)).longValue());

            row = result.get(2);
            assertEquals("count", 6L, ((Number)row.get(0)).longValue());
            assertEquals("sum", 9L, ((Number)row.get(1)).longValue());
            assertEquals("avg", 1.5d,  ((Number)row.get(2)).doubleValue(), 0.001);
            assertEquals("min", 1, ((Integer)row.get(3)).intValue());
            assertEquals("max", 2, ((Integer)row.get(4)).intValue());
        }
    }
