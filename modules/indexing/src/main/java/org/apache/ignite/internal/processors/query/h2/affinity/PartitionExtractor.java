...
public class PartitionExtractor {
    ...
    private PartitionNode tryExtractBetween(GridSqlOperation op, PartitionTableModel tblModel)
        ...
        // Check columns type
        if (!(leftCol.column().getType().getValueType() == Value.BYTE ||
            leftCol.column().getType().getValueType() == Value.SHORT ||
            leftCol.column().getType().getValueType() == Value.INT ||
            leftCol.column().getType().getValueType() == Value.LONG))
            return null;
        ...
        for (long i = leftLongVal; i <= rightLongVal; i++) {
            int part = partResolver.partition(i , leftCol.column().getType().getValueType(), tbl0.cacheName());
