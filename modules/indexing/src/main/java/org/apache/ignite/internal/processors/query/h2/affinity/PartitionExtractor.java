...
public class PartitionExtractor {
    ...
    private PartitionNode tryExtractBetween(GridSqlOperation op, PartitionTableModel tblModel)
        ...
        // Check columns type
        if (!(leftCol.column().getType() == TypeInfo.TYPE_BYTE || leftCol.column().getType() == TypeInfo.TYPE_SHORT ||
            leftCol.column().getType() == TypeInfo.TYPE_INT || leftCol.column().getType() == TypeInfo.TYPE_LONG))
            return null;
