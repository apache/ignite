...
public class GridSqlFunction extends GridSqlElement {
    ...
    @Override public String getSQL(boolean hideConst) {
        ...
        if (type == CASE) {
            buff.append(' ').append(child().getSQL(hideConst));

            for (int i = 1, len = size() - 1; i < len; i += 2) {
                buff.append(" WHEN ").append(child(i).getSQL(hideConst));
                buff.append(" THEN ").append(child(i + 1).getSQL(hideConst));
            }

            if ((size() & 1) == 0)
                buff.append(" ELSE ").append(child(size() - 1).getSQL(hideConst));
        ...
        switch (type) {
            ...
            case EXTRACT:
                ...
                buff.append(v.getString()).append(" FROM ").append(child(1).getSQL(hideConst));
            ...
            case TABLE:
                for (int i = 0; i < size(); i++) {
                    ...
                    buff.append(Parser.quoteIdentifier(((GridSqlAlias)e).alias()))
                        ...
                        .append('=')
                        .append(e.child().getSQL(hideConst));
                }
