...
public final class GridSqlType {
    ...
    public static GridSqlType fromColumn(Column c) {
        if (c.getName() != null)
            c = new Column(null, c.getType());

        return new GridSqlType(c.getType().getValueType(), c.getType().getScale(), c.getType().getPrecision(), c.getType().getDisplaySize(), c.getCreateSQL());
    }
