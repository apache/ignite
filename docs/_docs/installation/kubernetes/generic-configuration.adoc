...
Thin client supports partition awareness feature that may speed up operations. If the feature enabled a thin client stores a map of Ignite nodes to cache partitions.
So in moment of a request client knows which node stores a required partition and send the request directly to the node.
If client is connected to Kubertenes cluster through Ingress it makes impossible to build such a map. Instead, one should configure a client with KubernetesConnectionConfiguration.
In this case client is able to connect to every pod in a cluster.
