...
...
----
The operator will adjust the k8 yaml config to include an init container if an init container image is present
present in the custom resource.

NOTE: An init container command is optional.