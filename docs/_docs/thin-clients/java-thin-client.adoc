...
The code sample below shows how to use a custom cache key mapper function to enable affinity awareness from a thin client side
if a cache already exists in a cluster and was created with custom AffinityFunction or AffinityKeyMapper.

NOTE: The Apache Ignite an internal thin client API is used. There is no guarantee that it will no be changed in future releases.
