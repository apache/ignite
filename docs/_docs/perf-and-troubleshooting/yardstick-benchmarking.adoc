...
NOTE: The Yardstick framework has been moved to a separate extensions repository.
...
The Yardstick framework is now maintained in a separate extensions repository.
To build it, clone the repository and run:

[source, shell]
----
git clone https://github.com/apache/ignite-extensions.git
cd ignite-extensions/modules/yardstick
mvn clean package -DskipTests
----

Artifacts will be available in the `modules/yardstick/target/assembly` directory.

For detailed build instructions, refer to the https://github.com/apache/ignite-extensions/tree/master/modules/yardstick[Yardstick extensions repository, window=_blank].
