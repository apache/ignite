...
Cluster health is assessed against the state and workload intended for the deployment.
A healthy cluster may undergo planned topology changes or temporary load spikes.
The important properties are that the cluster serves the operations allowed by its intended state, data remains consistent, expected nodes are present, and transient activity such as rebalancing or queue growth converges instead of accumulating.
There is no universal single health metric.
...
For deployments that use the link:clustering/baseline-topology[baseline topology], check the baseline as well:
...
Changing the baseline can start link:data-rebalancing[rebalancing].

A temporary topology change does not make the cluster unhealthy by itself.
...
A successful `idle_verify` result is an important confirmation of consistency, but it does not prove application availability, the intended cluster state, lack of queue pressure, or absence of split brain.

For more details, see link:tools/control-script#verifying-partition-checksums[Verifying Partition Checksums].
...
You can also track rebalance metrics through `SYS.METRICS`:
...
WHERE NAME = 'cluster.Rebalanced'
   OR NAME LIKE 'cacheGroups.%.LocalNodeMovingPartitionsCount'
   OR NAME LIKE 'cacheGroups.%.LocalNodeRentingPartitionsCount'
ORDER BY NAME;
----

Do not interpret `cluster.Rebalanced=false` for an `INACTIVE` cluster without checking the cluster state: the link:monitoring-metrics/new-metrics#cluster[metric documentation] notes that an inactive cluster always reports this metric as `False`.
For related metrics, see link:monitoring-metrics/new-metrics-system#monitoring-rebalancing[Monitoring Rebalancing] and link:monitoring-metrics/new-metrics#cache-groups[cache group metrics].

=== Check execution queues

Ignite has several internal executors.
A regular thread pool executes tasks from a shared queue.
...
Short non-zero queues are acceptable under load.
...
The striped executor does not use the regular `QueueSize` attribute; use `TotalQueueSize`.
...
Do not expose unauthenticated remote JMX in production; use local access or your approved secured JMX setup.
