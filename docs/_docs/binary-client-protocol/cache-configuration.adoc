...
Upon successful handshake with an Ignite server node, a client can start performing various cache configuration operations by sending a request (see request/response structure below) with a specific operation code:
