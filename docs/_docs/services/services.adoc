...
To do that, enable link:../thin-clients/java-thin-client.adoc#java_thin_client_partition_awareness[Partition Awareness].

When used, the thin client knows about nodes with the service instances and invokes them directly.

Without Service Awareness, the invocation requests are sent to a random node. If it has no service
instance deployed, the request is redirected to a proper node. Such redirection costs.

[NOTE]
====
Obtaining and updating of the service topology takes some time. If a service proxy invokes a service for the first
time or if the service has migrated to another node, several service invocation requests may be still redirected until
the client gets actual service deployment map.
====
