...
[NOTE]
Overloaded methods have same metric by the method name.
...
[NOTE]
Method measuring requires additional wrapping and slows down the invocation. Probably it's not
an issue for slow-by-nature jobs like working with a DB. But, if you enable service statistics, even local services
become proxies which are slower.
