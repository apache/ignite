...
WARNING: Be aware that measuring method requires additional wrapping and slows the invocation. Probably it's not
an issue for slow-by-nature jobs like working with a DB. Also, if you enable service statistics, even local services
become proxies which are slower. See the example above.
