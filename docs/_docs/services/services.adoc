...
* `init(ServiceContext)`: called by Ignite before the service is deployed (and before calling the `execute()` method)
...
You can deploy services at runtime via the instance of `IgniteServices`, which can be obtained using the `Ignite.services()` method.
...
When you obtain the `IgniteServices` interface by calling `ignite.services()`, the `IgniteService` instance is associated with all server nodes.
...
If you want to update the implementation of a service without stopping the cluster, you can do it if you use the Ignite's link:deploying-user-code[DeploymentSPI configuration].
