...
Every time a new node is started (in a separate pod), the IP finder will connect to the service via the Kubernetes API to obtain the list of the existing pods' addresses.
...
:kubernetes-ip-finder-description: This IP finder will connect to the service via the Kubernetes API and obtain the list of the existing pods' addresses. Using these addresses, the new node will be able to discover all other cluster nodes.
...
----
include::{configDir}/stateless/deployment-template.yaml[]
----
...
[source,yaml,subs="attributes,specialchars"]
----
include::{configDir}/stateful/statefulset-template.yaml[]
----
