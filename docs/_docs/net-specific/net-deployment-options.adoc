...
To disable this default build action, add `<ApacheIgniteNoLibs>True</ApacheIgniteNoLibs>` property to a `<PropertyGroup>` in your `.csproj` file.
Alternatively, add or update `Directory.Build.props` file to do that for multiple projects.
This can be useful for thin client only use cases and custom deployments.
