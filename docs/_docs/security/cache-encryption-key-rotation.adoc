...
The process of changing the cache encryption key consists of two stages:

1. _Rotate cache group key_, this process adds a new encryption key for the specified cache group(s) on each server node and sets it to write new data.
2. _Background re-encryption_ of existing (archived) cache data.

Re-encrypting existing data can take a long time, so during this period the old key will be kept to read the archived data.
...
== Changing cache encryption key

=== Limitations

* Node join during _the rotation of the encryption key_ is prohibited and will be rejected. During re-encryption of existing data, which begins after the encryption key is rotated, the node join is not rejected and the cluster remains fully functional.
* Secondary rotation of the encryption key of a cache group is only possible after background re-encryption of existing data in that cache group has been completed.
...
== Background re-encryption management
