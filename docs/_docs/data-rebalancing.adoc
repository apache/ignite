...
By default, rebalancing is performed on the number of threads defines as `min(4, max(1, AVAILABLE_PROC_CNT / 4))` per node. For example, with 16 cotes (`AVAILABLE_PROC_CNT`= 16), this would compute to 4 threads.

You can increase the number of threads taken from the system thread pool and used for rebalancing.
