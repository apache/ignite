...
The H2-based SQL engine does not support non-recursive `WITH` clauses. The Calcite-based SQL engine supports recursive CTEs with the standard `WITH RECURSIVE` syntax and also supports omitting `RECURSIVE` as an Ignite compatibility extension; see link:SQL/sql-calcite#recursive-ctes[Recursive CTEs, window=_blank].
