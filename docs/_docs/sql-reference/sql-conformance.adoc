...
The H2-based SQL engine does not support non-recursive `WITH` clauses. The Calcite-based SQL engine supports recursive CTEs with `WITH RECURSIVE` and `UNION ALL` or `UNION DISTINCT`; see link:SQL/sql-calcite#recursive-ctes[Recursive CTEs, window=_blank].
