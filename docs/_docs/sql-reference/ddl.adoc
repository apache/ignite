...
NOTE: An index name is unique per schema. For example, if there are several tables inside some schema, then you cannot create indexes with the same names, even if each of the indexes will be created on different table.
