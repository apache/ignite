...
Usually all the cluster nodes are running on different machines and having the snapshot data spread across the cluster.
...
For such cases, the Apache Ignite ships with the built-in snapshot consistency check commands that enable you to verify
internal data consistency, calculate data partitions hashes and pages checksums, and printing out the result if a
