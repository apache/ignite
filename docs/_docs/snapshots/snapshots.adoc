...
2. provide a lightweight creation process that can be run concurrently with runtime load.

Incremental snapshots consist of compressed WAL segments that are collected in background without pressure on cluster resources.
...
During incremental snapshot restoring procedure the full snapshot is restored first and after that all increments are restored sequentially.
