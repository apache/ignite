...
* The operation requires the snapshot administration pesmissins via `IgniteSecurity` (if configured).
...
* If nodes share the same snapshot directory, a deletion concurrency may apper. Some nodes may not find snapshot at all
or decide that the removal wasn't complete. Snapshot is deleted anyway.
* The removal operation is not subject for the snapshot operation status and cancel requests.
