...
In a nutshell, you need to stop the cluster, replace persistence data and other files with the data from the snapshot,
...
* Cache groups to be restored from the snapshot must not present in the cluster, if they present, they must be destroyed by the user before starting this operation.
* Restore to a smaller topology is not supported. Those. restore is possible only if all parts of the snapshot are present in the cluster.
* Concurrent restore operations are not allowed. Those. if one restore operation has been started, the other can only be started after the first is completed.
