...
# Delete the snapshot "snapshot_09062021" located in the "C:\tmp\ignite\snapshots" folder.
control.bat --snapshot delete snapshot_09062021 --src C:\tmp\ignite\snapshots
----
--
...
* The deletion is rejected if any snapshot operation (create, restore, check, delete) is active for the snapshot.
* The operation requires the snapshot administration permissions via `IgniteSecurity` (if configured).
...
* The deletion operation is independent of the status of the snapshot operation and cancellation requests.
