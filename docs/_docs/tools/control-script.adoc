...
The command has two forms, and both answer with one line per node. With `--dry-run` it changes nothing and says
...
[source,text]
----
46db5416-d199-48d6-a807-88b7f1300000: can be reloaded client connector; would fail on communication (No trusted certificate found), discovery (No trusted certificate found); serving CN=node01 until 2039-11-07
----

For communication and discovery, a certificate that the node's own trust store rejects counts as unusable. This
assumes every node runs the same key and trust store configuration. The same answer comes back when the trust
stores hold the node certificates themselves rather than the certificate of an authority, which is the scheme the
link:security/ssl-tls[SSL/TLS] page describes; replacing a certificate is then a change of trust, so give the new
one to every trust store first, the way the next section gives them a new authority.
...
* create a key pair and a request for the new authority to sign. Keytool asks for a store password and then for a
key password: give the one the node is configured with to both, because it opens the store and the private key
with that one:
+
.Example
[source, shell]
----
keytool -genkeypair -keystore node.jks.new -alias node -keyalg RSA -dname "CN=node01"
keytool -certreq -keystore node.jks.new -alias node -file node.csr
----
* install what the authority returns for `node.csr`. A reply that carries the whole chain goes in as it is; one
that carries the node certificate alone needs the authority's own certificate imported first, or there is nothing
to chain the reply to:
+
.Example
[source, shell]
----
...
Keep one key entry per store: if it holds several, the JSSE key manager picks one of them and you cannot influence
which.
