...
You can use the control script to destroy specific user-created caches.

[source, shell]
----
control.sh|bat --cache destroy [cacheName1,...,cacheNameN] [--destroy-all-caches] [--skip-existence-check]
----
...
|===
| Parameter | Description
| `cacheName1,...,cacheNameN`| The names of the caches to be destroyed.
...
control.sh|bat --cache destroy cache1,cache2

# Destroy all user-created caches.
control.sh|bat --cache destroy --destroy-all-caches

# Execute destroy command for cache1 and cache2 even if one of these caches doesn't exist.
control.sh|bat --cache destroy cache1,cache2 --skip-existence-check
----
