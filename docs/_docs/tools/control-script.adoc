...
====
[discrete]
`idle_verify` command uses also own utility thread pool of default size `Total CPUs - 2` with the minimum of `4` threads.
The pool size might be adjusted at node start with the system property

`-DIGNITE_IDLE_VERIFY_POOL_SIZE=...`.
[discrete]
These pool threads expires after some time being unused.
====
