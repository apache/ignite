...
The command reaches what is in the topology at the moment it is started, and nothing else. Applications are never reached, which
...
Everything reloads except two arrangements, which the first report names as `cannot be reloaded` and the second
as `not reloaded`, rather than as a success:
...
without putting any of them in use, and reports what each of them would then serve:

[source,text]
----
e1e18f84-0fb5-484d-a623-15ffe3800000: can be reloaded client connector, communication, discovery; will serve CN=node01 until 2041-02-14, issued by CN=ca
0cf9e4e2-bb7a-466a-a113-b6230b500001: can be reloaded client connector, communication, discovery; will serve CN=node01 until 2041-02-14, issued by CN=ca
----
...
Nearly everything that can fail happens in this phase; the one thing that can fail later is named further down. A
node that would not be able to use the new certificate says so, and the command stops without having changed
anything on any node:

[source,text]
----
e1e18f84-0fb5-484d-a623-15ffe3800000: can be reloaded client connector, communication, discovery; will serve CN=node01 until 2041-02-14, issued by CN=newca
46db5416-d199-48d6-a807-88b7f1300000: would fail on client connector, communication, discovery (No trusted certificate found)
----

Every node is in the report either way, so it also shows which of them were ready. The command ends with an error,
not with a result.
...
[source,text]
----
e1e18f84-0fb5-484d-a623-15ffe3800000: reloaded client connector, communication, discovery; serving CN=node01 until 2041-02-14, issued by CN=ca
0cf9e4e2-bb7a-466a-a113-b6230b500001: reloaded client connector, communication, discovery; serving CN=node01 until 2041-02-14, issued by CN=ca
----

A node that joined while the question was on screen has nothing prepared, is reported as not reloaded, and keeps
the certificates it read when it started. A node that left is reported as having left, which on its own does not
fail the command. Run the command again once the topology has settled.

The HTTP REST connector is the one part that rebuilds in place instead of holding the new certificates aside, so
it is also the only one that can still fail in this phase. It is then reported as failed and keeps serving the
certificate it had, while everything else on that node, and on every other node, has already moved. It carries no
traffic between nodes, so a failure there cannot split the cluster.
...
The report is the same one, so it also names the certificate each node would serve:
...
+
--
*Nodes.* Step 2 has already answered this for every node that was in the topology: its report names the authority
that issued what each of them serves.
...
*Once for the whole cluster.* A node that never got its new certificate is refused here, because its own trust
store no longer accepts what it presents. Read the report and answer the question it asks:
