...
They print a complete result on success or fail with an exception; partial results are not returned.

=== Communication SPI Diagnostic

The `communication` test sends request-response messages from the selected source node to every remote server node through the Communication SPI.
...
WARNING: These tests generate network traffic and consume CPU on participating nodes; high thread counts, long durations, or large payloads can affect cluster workloads.
The `--process-in-nio-thread` communication option handles test messages directly on network I/O threads and should only be used during a controlled diagnostic run because it can delay normal network processing.

NOTE: Estimated one-way Communication SPI delivery times and per-hop Discovery SPI delays compare wall clocks on different nodes and require synchronized clocks.
Use RTT and full-ring latency when clocks are not synchronized.
