...
====
[discrete]
`idle_verify` command also establish own utility thread pool of default size `Total CPUs - 2` with the minimum of `4` threads.
The pool size might be adjusted at node start with the system property

`-DIGNITE_IDLE_VERIFY_POOL_SIZE=...`.
[discrete]
The pool threads exist idle for some time after a verify task.
====
