...
[source, shell]
----
control.sh|bat --cache create --springXmlConfig springXmlFilePath --exclude-caches cache1,cache2,cache3
----
...
|`--exclude-caches cache1,cache2,cache3`| Optional flag to list caches to ignore at creation step. You can use regular expressions to list caches.
|===
...
# Create caches from the `/ignite/config/userCaches.xml` configuration. Ignores cache1, cache2, cache3 and noopCache
control.sh|bat --cache create --springXmlConfig /ignite/config/userCaches.xml --exclude-caches cache[1-3],noopCache`
----
