...
== Getting Arbitrary System View

To get an arbitrary system view, execute the following command:
...
[source,shell,subs="verbatim,quotes"]
----
control.sh --system-view system_view_name [–node-id node_id]
----
tab:Windows[]
[source,shell,subs="verbatim,quotes"]
----
control.bat --system-view system_view_name [–node-id node_id]
----
--
...
|===
| Parameter | Description
| `--system_view_name` a| Name of the requested value system view. Both SQL and common style system view names are supported. For example, `SQL_TABLES` and `sql.tables` are handled similarly.
| `--node_id` a| ID of the node to get the system view from. If the parameter is not set, a random node is chosen.
|===
