...
====
[discrete]
`idle_verify` command uses its own utility thread pool of default size `Total CPUs - 2` with the minimum of `4` threads.
...
The metrics command configures bounds of histogram metrics, rate time interval of hitrate metrics, or time interval of max value metrics.
...
control.sh --metric --configure-max-value max-value-metric-name 1000
----
tab:Windows[]
[source,shell,subs="verbatim,quotes"]
----
...
control.bat --metric --configure-max-value max-value-metric-name 1000
----
--
