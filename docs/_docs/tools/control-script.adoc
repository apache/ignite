...
Refer to the link:events/listening-to-events#enabling-events[Enabling Events] section for details.
====
...
The command uses special strategies to perform the repair. It's recommended to use `CHECK_ONLY` strategy to list inconsistent values and then chose the proper link:key-value-api/read-repair#strategies[Repair Strategy].
