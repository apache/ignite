...
The checking snapshot consistency command working the same way as the `idle_verify` command does. It compares hashes between
a primary partition and a corresponding backup partitions and prints a report any differences occurs.
...
[source,shell]
----
# Checks partitions of all snapshot caches have correct checksums and primary/backup actually contain the same data.
