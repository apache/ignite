...
| --password +
  ...
  --keystore-password +
  +
  --truststore-password | Forces the command to prompt for a password before connecting to a cluster. +
  This option ensures secure authentication by requiring a password input. Specifying the password through this argument prevents exposing sensitive information on the command line or in configuration files. |
|===
