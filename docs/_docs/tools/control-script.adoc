...
The command has two forms. With `--dry-run` it changes nothing and says what each node would do:
...
A node that would not be able to use the new certificate says so, and the command fails:
...
Worth doing before every reload: the command reaches the nodes independently of each other, so a node that cannot
use its new certificate otherwise shows up only once the others have already switched.

Without `--dry-run` the certificates are put in use, and the same lines report what happened:
...
A node that cannot use the new certificate keeps the old one, names the transport it failed on, and the command
fails. Nothing is rolled back, and nothing has to be: fix the files on that node and run the command again. A
second run is safe, and a node that already serves the new certificate simply re-reads the same files.
