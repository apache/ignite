...
*Once for the whole cluster.* A dry run first is optional, and pays off here more than anywhere else: this is the
step that fails a node whose new key store its own trust store does not accept, which is what a skipped step 1
looks like on that node. It changes nothing.
...
Read the report, then put the certificates in use on the nodes:
