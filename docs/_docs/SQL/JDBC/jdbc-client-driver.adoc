...
|Presently ACID Transactions are NOT supported. At the SQL level, Ignite supports atomic, but not transactional consistency.

This means that the JDBC driver will throw a `Transactions are not supported` exception if you try to use this functionality.
