...
with other SQL operators. Custom SQL function is also a public static method marked by the same `@QuerySqlFunction`
annotation but with setting additional `tableColumnTypes` and optionally `tableColumnNames` parameters.
Table function must return `Iterable` as a row set. Each row can be represented by `Object[]` or
by `Iterable` too. Row length must match the defined number of column types. Row value types must match the
...
----
NOTE: The table functions also are available currently only with Calcite.
