...
one _SELECT_ or one table *are not allowed*.
...
SELECT T1.* FROM TBL1 /*+ NO_INDEX */ where T1.V1=? and T1.V2=?
----
...
=== Hint scope
Hints of _SELECT_ are "visible" for this operation and the following relation operators, queries and subqueries.
Hints in a subquery have effective scope only for this subquery, its subqueries. Hint, defined for a table, is effective
only for this table.
...
SELECT T1.V1 FROM TBL1 T1 JOIN TBL2 /*+ FORCE_INDEX(TBL2_IDX3) */ T2 ON T1.V2=T2.V2 and T1.V3=T2.V3 and T2.V3=?;
----
...
=== Hints priority
Hints, defined in subqueries or in following _SELECTs_ has priority over the preceding ones. In the following example
an index for _TBL2_ is actually applied.
[source, SQL]
----
SELECT /*+ NO_INDEX */ * FROM TBL1 T1 WHERE T1.V1 = (SELECT /*+ FORCE_INDEX(IDX_TBL2_V3) */ T2.V1 FROM TBL2 T2 where T2.V2=? and T2.V3=?)
----
Table hints have priority over _SELECT_ hints. In the following example an index for _TBL1_ is actually applied.
[source, SQL]
----
SELECT /*+ NO_INDEX */ * FROM TBL1 /*+ FORCE_INDEX(IDX_TBL1_V2) */ T1 where T1.V1=? and T1.V2=? and T1.V3=?;
----
