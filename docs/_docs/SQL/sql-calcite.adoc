...
[source, SQL]
----
SELECT /*+ FORCE_INDEX(IDX_TBL1_V2), EXPAND_DISTINCT_AGG */ T1.V2, AVG(DISTINCT T1.V3) FROM TBL1 WHERE T1.V1=? and T1.V2=? GROUP BY T1.V2

SELECT T1.* FROM TBL1 /*+ FORCE_INDEX(IDX_TBL1_V2) */ where T1.V1=? and T1.V2=?
----
...
=== Hints priority
Hints, defined in subqueries or in following _SELECTs_, have priority over the preceding ones. In the following example
...
----
Table hints have a bigger priority. In the following example an index for _TBL1_ is actually applied.
[source, SQL]
----
