...
Functions marked as Ignite-specific are provided through Ignite's Calcite operator table.
...
|`LENGTH`
|`LENGTH(string)`
|Ignite-specific alias for `CHAR_LENGTH(string)`.
...
For more information on standard Calcite functions and syntax variants, please see the link:https://calcite.apache.org/docs/reference.html#operators-and-functions[Apache Calcite SQL language reference, window=_blank].
