...
The Calcite-based SQL engine supports recursive common table expressions (CTEs) with the standard `WITH RECURSIVE` syntax.
As an Ignite compatibility extension, the engine can also infer recursion when the `RECURSIVE` keyword is omitted and the recursive term directly references its own CTE.
Both forms support `UNION ALL` and `UNION` or `UNION DISTINCT`.
...
In the following example, you can omit the `RECURSIVE` keyword without changing the result.
...
The recursive part can directly reference the recursive CTE more than once and can contain multiple recursive branches.
A self-reference nested in a scalar or derived-table subquery is not supported.
`UNION DISTINCT` removes duplicate rows from the seed rows and across recursive iterations.
