...
hint name are required. The hint block is placed right after _SELECT_ or after a table name if hint is suitable for
a table. Several hint blocks for one _SELECT_ or one table *are not allowed*.
...
=== Hints scope
Hints are defined for _SELECT_ relation operator and are "visible" for the following operators, queries and subqueries.
