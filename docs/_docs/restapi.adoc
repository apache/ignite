...
NOTE: Ignite uses Jetty 12. Custom Jetty XML configurations created for earlier Jetty versions must use an unnamed
`<Arg>` element for the server thread pool and must not define the legacy `HandlerCollection` and
`ContextHandlerCollection`. The following configuration uses the current syntax.
