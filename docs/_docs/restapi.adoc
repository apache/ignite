...
Note that unless one of the mentioned below types are explicitly specified, the REST protocol exchanges the key-value data in `String` format.
...
* If the type name is a `Java class` available on the server, the type of each field of the object will be resolved according to the type of that field in the `Java class`.
...
=== Cluster State
Returns the current link:cluster-states[state of the cluster].
...
The `setstate` command changes the link:cluster-states[cluster state].
