...
For partition-aware read operations, the client sends requests to a node from its own data center when the
cache has `readFromBackup=true` and the local data center has a partition copy.
