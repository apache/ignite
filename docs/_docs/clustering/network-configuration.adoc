...
give you more reliability. Several node addresses can prolong detection of node failure. By default, node obtains all
available IP addresses. Connection to node is established to first non-loopback. But if lost, other addresses are
sequentially attempted to reconnect to.
