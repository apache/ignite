...
*** `ComputeTaskAdapter` — This class implements the `result()` method as follows:
** In case of successful job executions without errors, the task waits until results are gathered from all jobs, computes the final result, and completes successfully (see `WAIT` policy).
** If AT LEAST ONE job fails:
* The failed job will be automatically redeployed on another node (`FAILOVER` strategy), under these conditions:
- The user-provided executor refuses to process the job (a `ComputeExecutionRejectedException` is caught).
- Something happened to the node where the job was supposed to run — it left topology or crashed (caught `ClusterTopologyException`)
- Job's user code explicitly throws an exception (caught `ComputeJobFailoverException`)
* If any other error occurs while executing the job, then the entire task immediately completes with failure by throwing an `IgniteException`, containing message "Remote job threw user exception...". Other jobs whose results have not yet been retrieved will be asynchronously terminated.

[WARNING]
====
Please note that Jobs interruption happens ASYNCHRONOUSLY — there’s no guarantee that at the moment when the task finishes, all jobs have completed their work. It is recommended to ensure that all jobs are properly terminated.
====
