...
* `ComputeTaskAdapter` — This class implements the `result()` method.
+
This implementation makes the best effort to wait for all Compute Jobs to complete successfully and process their results (see `WAIT` policy).
+
--
Comute Jobs will be automatically redeployed (see `FAILOVER` policy) in the following scenarious:
...
--
+
If any of the mapped Compute Jobs threw a user exception, the entire Compute Task immediately fails and all remaining Compute Jobs are stopped asynchronously. In this case, there is no guarantee that all Compute Jobs complete execution by the end of Compute Task.
+
Compute Job failures can be tracked in the Ignite log for the message "Remote job throws user exception...".
+
[WARNING]
====
Please note that Jobs interruption happens asynchronously — there’s no guarantee that at the moment when the task finishes, all jobs have completed their work.
====
