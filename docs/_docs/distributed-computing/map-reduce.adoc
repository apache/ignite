...
- Returns the `FAILOVER` policy if the thrown exception is an instance of `ComputeExecutionRejectedException`, `ClusterTopologyException` or `ComputeJobFailoverException`;
- Throws an `IgniteException("Remote job threw user exception (override or implement ComputeTask.result(..) method if you would like to have automatic failover for this exception): " + e.getMessage(), e);` if the exception is not an instance of the above-listed classes;
- Returns the `WAIT` policy otherwise. It means that this implementation will wait for all jobs to finish with a result.
