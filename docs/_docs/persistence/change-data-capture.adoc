...
link:#flush-existing-caches[flush caches command], snapshot or other methods.
...
In this case it is necessary to flush data from existing caches. For example, this is important for the following cases:
...
The command will iterate over caches and writes data entries to the WAL to get captured by CDC application.
