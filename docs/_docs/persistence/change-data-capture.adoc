...
| `DataRegionConfiguration#cdcWalPath` | Path to the CDC directory | `"db/wal/cdc"`
| `DataRegionConfiguration#walForceArchiveTimeout` | Timeout to forcefully archive the WAL segment even it is not complete. | `-1` (disabled)
|===
