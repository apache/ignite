...
In this case it is necessary to resend data from existing caches. For example, this is important for the following cases:
...
[source,shell]
----
# Forcefully resend all cache data to CDC. Iterates over caches and writes data entries to the WAL to get captured by CDC:
control.sh|bat --cdc resend --caches cache1,...,cacheN
----

The command will iterate over caches and writes data entries to the WAL to get captured by the CDC application.
