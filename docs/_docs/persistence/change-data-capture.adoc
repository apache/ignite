...
The CDC can be disabled manually or by configured directory maximum size threshold. In this case a hard link creation will be skipped.
...
The command will remove all segment links before the last gap. For in-memory mode state will be reset to the first record written after CDC enable again.
...
== Handling disabled in-memory CDC

When in-memory CDC disabled, the special WAL record (CDC disabled record) will be written on data changes.

WARNING: All changes are skipped when in-memory CDC is disabled.

The CDC application will fail if meets this record while processing segments. Example of error:
`Found the CDC disabled record. Some events are missed. Exiting! [state=WALPointer [idx=1, fileOff=29, len=21]]`

It means that some data changes are skipped and administrator attention is required.

You can reset the CDC application state to the first record written after CDC enable again and fix possible gaps
using the `delete_lost_segment_links` link:#handling-skipped-segments[command].
