...
When in-memory CDC disabled, the special WAL record (CDC disable record) will be written on data changes.
...
`Found the CDC disable record. Some events are missed. Exiting! [state=WALPointer [idx=1, fileOff=29, len=21]]`
