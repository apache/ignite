...
`ignite-cdc.sh` uses the same link:monitoring-metrics/new-metrics-system#metric-exporters[metric exporters] as an Ignite node.
They are configured by the `metricExporterSpi` property of `CdcConfiguration`, the JMX exporter is used by default.
The application registers its metrics in the `cdc` register.
The `cdc.consumer` register is passed to `CdcConsumer.start(MetricRegistry)` and is reserved for the consumer-specific metrics.
...
|BinaryMetaDir | string | Binary metadata directory the application reads data from.
|CdcDir | string | CDC directory the application reads data from.
|CdcMode | string | CDC mode: `CDC_UTILITY_ACTIVE` when the events are captured by the application, `IGNITE_NODE_ACTIVE` when they are captured within the Ignite node.
|CommittedSegmentIndex | long | Index of the WAL segment that contains the last committed state.
|CommittedSegmentOffset | long | Committed offset in bytes inside the WAL segment.
|CurrentSegmentIndex | long | Index of the WAL segment being processed.
|EventCaptureTime | histogram | Time between the event creation on the Ignite node and its capture by the consumer, in milliseconds.
|EventsCount | long | Number of events passed to the consumer.
|LastEventTime | long | Timestamp (in milliseconds) of the last event passed to the consumer.
|LastSegmentConsumptionTime | long | Timestamp (in milliseconds) indicating the last segment processing start.
|MarshallerDir | string | Marshaller directory the application reads data from.
|MetadataUpdateTime | histogram | Time of the metadata update (marshaller mappings, binary types and cache configurations), in milliseconds.
|SegmentConsumingTime | histogram | Time of WAL segment consumption by consumer, in milliseconds.
|===
...
Detailed documentation can be found on link:extensions-and-integrations/change-data-capture/overview[page].
