...
flush caches command, snapshot or other methods.
...
In some cases when the CDC has been forcefully disabled for a while, it is necessary to flush data from
existing caches, for example:

- pre-synchronize replication;
- handle missed segments.

To flush cache entries you can run the following link:tools/control-script[Control Script] command:

[source,shell]
----
# Flush caches.
control.sh|bat --cdc flush_caches --caches cache1,...,cacheN [--only-primary]
----

The command will iterate over caches and writes data entries to the WAL to get captured by CDC.
