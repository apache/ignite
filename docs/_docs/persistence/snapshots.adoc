...
* The `binary_meta` and `marshaller` directories store metadata and marshaller-specific information needed for the restore.
...
it's not located under the Ignite `work` dir.
    - Copy the files of belonging to a node with the `{node_id}` from the snapshot into the `$IGNITE_HOME/work/` directory.
...
might require extra space on the Ignite Persistence storage media (up to the current size of it).
