...
Ignite DataStreamer is an orchestrator. It doesn't write data itself. link:{javadoc_base_url}/org/apache/ignite/stream/StreameReceiver.html[StreamerReceiver] does.

The default receiver is designed for fastest data load and fewer network requests. With this receiver, streamer focuses
on parallel transfer of backup and primary records. As result, primary records might be accepted and written after the
backups and won't be visible in your requests.

You can set your own receiver. See

Default doesn't overwrite existing keys.

You can change that behavior by setting the `allowOverwrite` property of the data streamer to `true`.
