...
The main goal of the streaming is efficient, quick data loading. The data that is added to the streamer is
...
You can put data into streamer with multiple threads.
...
[]
- Data order. Data records may be load with different order that it was put into streamer;
...
If <<overwritting, 'allowOverwrite'>>  property is 'false' (default), you should avoid:
[]
- Having the same keys repeating in the data being streamed;
- Streamer cancelation or streamer node failure;
- Using data loaded with canceled or failed streamer;
- Creating snapshots while streaming.

Most important behaviour of Ignite Data Streamer is defined by <<receivers, stream receiver>> and
