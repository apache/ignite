...
Ignite provides a Data Streaming API that can be used to inject large amounts of data into an Ignite cluster. The main goal of the streaming is efficient, quick data loading. The data that is added to the streamer is automatically organized and distributed between the nodes in partition-aware and parallel manner.

image:images/data_streaming.png[Data Streaming]

The Data Streaming API is designed to be scalable and fault-tolerant, and provides _at-least-once_ delivery semantics
for the data streamed into Ignite, meaning each entry is processed at least once.

Good use of streaming is data preloading.

== Limitations
Ignite data streamer doesn't guarantee:
[]
- Data order. Data records may be put in cache with dirrefent order than in streamer.
- Immediate data loading. Data can be kept for a while before sending.
- In general, data consistency until finished.
- Working with external storages.

You should avoid:
[]
- Having the same keys repeating in the data being streamed;
- Concurrent updates of the cache that is being streamed into;
- Rebalance on that cache;
...
In the Java version of Ignite, a data streamer is an implementation of the `IgniteDataStreamer` interface. `IgniteDataStreamer` provides a number of `addData(...)` methods for adding key-value pairs to caches. Refer to the link:{javadoc_base_url}/org/apache/ignite/internal/processors/datastreamer/DataStreamerCacheUpdaters.html[IgniteDataStreamer] javadoc for the complete list of methods.
...
Ignite Streamer is an orchestrator and doesn't write data itself. link:{javadoc_base_url}/org/apache/ignite/stream/StreameReceiver.html[StreamerReceiver] does. The default receiver is designed for fastest data load and fewer network requests. With this receiver, streamer focuses on parallel transfer of backup and primary records.
...
Changing receiver to non-default changes data distribution algorythm. With non-default receiver streamer sneds data batches only to primary nodes where reciver acceps them. This is slower if cache has backups. In this case primary node needs another request or requests to write backup.
...
By default, existing keys aren't overwritten. You can change that behavior by setting the `allowOverwrite` property of the data streamer to `true`. Since the default receiver does not overwrite data, other provided one is automatically chosen. Any non-default reciver is considered as not-overwritting. And `allowOverwrite` property says `false`.
