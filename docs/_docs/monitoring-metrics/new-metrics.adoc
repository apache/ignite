...
|RejectedSslSessions|   integer|   TCP sessions count that were rejected due to the SSL errors (metric is exported only if SSL is enabled).
...
|ReceivedBytesCount|   long|   Received bytes count.
|RejectedSslSessions|   integer|   TCP sessions count that were rejected due to the SSL errors (metric is exported only if SSL is enabled).
|RejectedConnectionsByTimeout|   integer|   TCP sessions count that were rejected due to handshake timeout.
|RejectedConnectionsDueAuthentication|   integer|   TCP sessions count that were rejected due to failed authentication.
|RejectedConnectionsTotal|   integer|   Total number of rejected TCP connections.
|{clientType}.AcceptedConnections|   integer|   Number of successfully established sessions.
|{clientType}.ActiveConnections|   integer|   Number of active sessions.
|SentBytesCount|   long|   Sent bytes count.
|SslEnabled|   boolean|   Indicates whether SSL is enabled.
|SslHandshakeDurationHistogram|   histogram|   Histogram of SSL handshake duration in milliseconds (metric is exported only if SSL is enabled).
|===
