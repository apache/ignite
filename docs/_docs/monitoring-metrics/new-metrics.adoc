...
|maxOutboundMessagesQueueSize|   long| Maximum number of outbound messages waiting to be sent during the configured metric interval.
...
|MaxMsgQueueSize|   long|   Maximum TCP discovery message worker queue size during the configured metric interval.
|MessageWorkerQueueSize|   integer|   Current message worker queue size.
|Next|   UUID|   Next node ID in the TCP discovery ring.
...
|SizeUsedByData| long|    Estimated number of bytes occupied by data in the region, taking into account free space inside non-empty pages.
