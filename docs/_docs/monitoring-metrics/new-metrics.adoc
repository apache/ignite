...
|===
|Name|    Type|    Description
|CoordinatorSince|   long|   Timestamp since which the local node was the coordinator (metric is only exported from server nodes).
|Coordinator|   UUID|   Coordinator ID (metric is only exported from server nodes).
