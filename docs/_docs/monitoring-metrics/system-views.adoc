...
|NODE_FILTER | VARCHAR | String representation of the node filter defined for the cache group.
...
|JOB_ID | UUID | Deprecated, use `ID` or `SESSION_ID` instead
|===
...
|TASK_CLASS_NAME | string | Class name of the task
...
|IS_TIMED_OUT | boolean | `True` if the job timed out before completing
...
|TOPOLOGY_SNAPSHOT | string | Number of service instances per node, as a map of node ID to instance count
|===
...
|FLAGS | short | All flags for the lock. This field represents a bit mask with possible values: LOCAL(0x01), OWNER(0x02), READY(0x04), REENTRY(0x08), USED(0x10), TX(0x40), SINGLE_IMPLICIT(0x80), DHT_LOCAL(0x100), NEAR_LOCAL(0x200), REMOVED(0x400), READ(0x800);
...
|NODE_ORDER |BIGINT |Node order within the topology.
...
The IGNITE_PLUGINS view contains information about ignite plugins configured on the node.
...
When using the H2 engine, collecting SQL plans may introduce a performance overhead under high SQL query load, impacting both latency and throughput. For this reason, SQL plan collection is disabled by default for H2.
...
When using the Calcite engine, collecting SQL plans does not impact performance. Therefore, it is enabled by default. The default history size is 1000 entries.
...
|MAX_JOBS_WAIT_TIME|BIGINT| Maximum time a job ever spent waiting in a queue before being executed, in milliseconds.
|CUR_JOBS_WAIT_TIME|BIGINT| Longest wait time among the jobs that are currently waiting for execution, in milliseconds.
|AVG_JOBS_WAIT_TIME|BIGINT| Average time jobs spend in the queue before being executed, in milliseconds.
|MAX_JOBS_EXECUTE_TIME|BIGINT| Maximum job execution time, in milliseconds.
|CUR_JOBS_EXECUTE_TIME|BIGINT| Longest time a current job has been executing for, in milliseconds.
|AVG_JOBS_EXECUTE_TIME|BIGINT| Average job execution time on this node, in milliseconds.
|TOTAL_JOBS_EXECUTE_TIME|BIGINT| Total time all finished jobs took to execute on this node since the node startup, in milliseconds.
...
|TOTAL_BUSY_TIME|BIGINT| Total time this node spent executing jobs, in milliseconds.
|TOTAL_IDLE_TIME|BIGINT| Total time this node spent idling (not executing any jobs), in milliseconds.
|CUR_IDLE_TIME|BIGINT| Time this node has spent idling since executing the last job, in milliseconds.
...
|UPTIME|BIGINT| Uptime of the JVM, in milliseconds.
...
|CACHE_GROUP_ID | int | Cache group id for the table
|CACHE_GROUP_NAME | string |  Cache group name for the table
...
The PAGES_TIMESTAMP_HISTOGRAM view shows how many pages of a data region were last accessed within each time interval.
It helps to estimate how much of the data loaded into memory is "hot" and how much is "cold".
The view is node-specific and contains rows only for the data regions with persistence enabled and link:monitoring-metrics/configuring-metrics#enabling-data-region-metrics[metrics enabled].
By default, the intervals are one hour long and 24 intervals are kept.
The first row of a region also counts the pages accessed before the earliest interval, the last row ends at the current time.
The same data is exposed by the `PageTimestampHistogram` metric of the link:monitoring-metrics/new-metrics#data-region-io[data region register].

[{table_opts}]
|===
|Column|Data Type|Description
|DATA_REGION_NAME | string | Data region name.
|INTERVAL_START | date | Start of the last-access time interval.
|INTERVAL_END | date | End of the last-access time interval.
|PAGES_COUNT | long | Number of pages last accessed within the interval.
|===
...
|INITIAL_COUNT | int | Initial count
...
|LAST_UPDATE_TIME |TIMESTAMP |Maximum time of all partition statistics which was used to generate local one.
|===
...
|SIZE |INTEGER |Average value size in bytes.
|VERSION |BIGINT |Statistics version.
|LAST_UPDATE_TIME |BIGINT |Time of the partition statistics collection, as a Unix timestamp in milliseconds.
|===

== STATISTICS_GLOBAL_DATA

The STATISTICS_GLOBAL_DATA view contains global link:SQL/sql-statistics[SQL statistics,window=_blank]: the statistics of a whole table, aggregated from the local statistics of the server nodes that own its partitions.
Global statistics of an object are gathered by one of the server nodes and delivered to the other nodes on request.
The view is node-specific and lists the global statistics cached on the node.
...
|SIZE |INTEGER |Average value size in bytes.
|VERSION |BIGINT |Statistics version.
|LAST_UPDATE_TIME |TIMESTAMP |Time of the global statistics aggregation.
|===
