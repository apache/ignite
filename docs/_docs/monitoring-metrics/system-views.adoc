...
The BASELINE_NODE_ATTRIBUTES view exposes node attributes stored for a current baseline topology in the metastorage.

[{table_opts}]
|===
| Column | Data Type | Description
| NODE_CONSISTENT_ID | VARCHAR | Node consistent ID.
| NAME | VARCHAR | Name of the user attribute.
| VALUE | VARCHAR | User attribute value.
