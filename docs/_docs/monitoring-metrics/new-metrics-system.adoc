...
| cache.myCache.CommitTime          | [0, 0, 0, 0, 0, 0]             |
....
----
...
* `RebalancingStartTime` - The time the first partition demand message was sent. If there are no messages to send, the rebalancing time will be undefined. Belongs to the `cacheGroups.{group_name}` register, see link:monitoring-metrics/new-metrics#cache-groups[Cache Groups].
* `RebalancingKeysRate`, `RebalancingBytesRate`, `EstimatedRebalancingKeys`, `RebalancedKeys` - Rebalancing progress of a cache, see link:monitoring-metrics/new-metrics#caches[Caches].
* `EstimatedRebalancingFinishTime` and `KeysToRebalanceLeft` - Available only through the `CacheMetrics` Java API (`IgniteCache.metrics()`); the latter equals `EstimatedRebalancingKeys - RebalancedKeys`.
...
. `IsIndexRebuildInProgress` - tells whether indexes are being rebuilt at the moment;
...
* `TransactionsCommittedNumber` - The number of transactions that have been committed on the node. Exposed as `txCommits` in the `tx` register.
* `TransactionsRolledBackNumber` - The number of transactions that were rolled back. Exposed as `txRollbacks` in the `tx` register.
