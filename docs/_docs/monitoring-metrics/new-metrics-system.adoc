...
The metric is calculated as `(TotalAllocatedPages - EmptyDataPages) * pageSize * PagesFillFactor`, which is equivalent to `TotalUsedSize * PagesFillFactor`. `PagesFillFactor` is the average ratio of data bytes to page size in non-empty data pages.
