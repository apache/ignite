...
To enable tracing of SQL queries, use `SQL` as value of `scope` parameter during link:[trace sampling configuration].
If tracing of SQL queries is enabled, execution of each SQL query on any cluster node will produce a separate link:[trace].

[IMPORTANT]
====
[discrete]
Enabling tracing for SQL queries impose severe degradation on SQL engine performance.
====

The table below provides description and a list of tags and annotations for each span that can be part of the SQL query trace tree.

[NOTE]
====
[discrete]
Depending on the type of SQL query and its execution plan, some spans can be not present in the SQL query span tree.
====

[cols="2,5,5",opts="header"]
|===
|Span Name | Description | Tags And Annotations
| sql.query | Execution of SQL query from the moment of registration until the release of used resources on the query initiator node a|
...
* sql.query.text - SQL query text
| sql.next.page.request | Processing of request for obtaining the next page of local SQL query execution result  |
| sql.page.response | Processing of message with page of node local SQL query execution result |
| sql.query.execute | Execution of query by H2 SQL engine a|
* sql.query.text - SQL query text
| sql.page.prepare | Reading rows from cursor and preparing result page a|
* sql.page.rows - Number of rows that result page contains
| sql.fail.response | Processing of message that indicates failure of SQL query execution |
...
* sql.index - Name of SQL index
* sql.table - Name of SQL table
* sql.index.range.rows - Number of rows that result of index range request contains
| sql.index.range.response | Processing SQL index range responce |
| sql.dml.query.execute |  Execution of SQL DML query |
| sql.command.query.execute | Execution of SQL command query which either DDL query or Ignite native command |
| sql.partitions.reserve | Reservation of data partitions used to execute a query a|
* Annotation message that indicates reservation of data partitions for particular cache - `Cache partitions were reserved [cache=<name of the cache>, partitions=[<partitions numbers>]`
