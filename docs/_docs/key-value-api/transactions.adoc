...
* Internal Apache Ignite bugs or Java-level deadlocks in Apache Ignite code. Indicators: thread dumps showing threads waiting for locks held by other threads; over time, the thread state doesn't change (stack trace remains unchanged). Not all deadlocks can be detected by thread dumps, for example, deadlocks caused by pool starvation. In such cases Apache Ignite has built-in pool starvation detection mechanism.
