...
[WARNING]
====
It is critical that an Ignite Transaction be `closed` regardless of its commit state. This ensures that all resources are released and the transaction is no longer bound to the current thread. Therefore, the `try-with-resource` block is highly recommended when working with transactions.
====
