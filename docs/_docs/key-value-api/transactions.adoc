...
[WARNING]
====
Apache Ignite requires calling the `close()` method after completing any transaction. We strongly recommend using transactions within a `try-with-resources` block to automatically close them when exiting the scope. If, however, it's impossible or impractical to utilize `try-with-resources`, always explicitly invoke the `close()` method inside a `finally` block to ensure proper cleanup and avoid resource leaks.
====
