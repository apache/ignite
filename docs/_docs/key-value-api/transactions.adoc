...
[WARNING]
====
When using the `try-catch-finally` construct, make sure to call the `close()` method in `finally` section for correct transaction closure, or use the `try-with-resources` block as shown in the example above.
====
